quest slime_dungeon begin
	...
	--QUEST FUNCTIONS
		...
		function create_dungeon()
			...
			server_timer("slime_dungeon_15minutesleft", 15*60, d.get_map_index())
		end
		--FUNCTIONS END

		--DUNGEON CHECK
		
		when 70007.use with pc.in_dungeon(240) begin
		...
		--MAP ENTER
		--DUNGEON ENTER
		when 9236.chat."Anime-Dungeon (20-55)"  with not slime_dungeon.is_slimed() begin
			local settings = slime_dungeon.settings()
			addimage(25, 10, "slime_dungeon.tga")
			addimage(230, 150, "thurang.tga")
			say("")
		...
		when kill with slime_dungeon.is_slimed() and not npc.is_pc() and npc.get_race() == 768 and d.getf("slimedung_level") == 8 begin
			...
			dungeon_info.set_ranking(DUNGEON_ID_SLIME_1, pc.getqf("boss_kills"), get_global_time() - pc.getqf("dungeon_start_time"), d.get_boss_max_dmg(npc.get_race()))
			pc.update_dungeon_progress(DUNGEON_ID_SLIME_1)
			d.completed()
			server_timer("slime_dungeon_is_done", 30, d.get_map_index())
		...
		when slime_dungeon_15minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				...
				server_timer("slime_dungeon_10minutesleft", 10*60, d.get_map_index())
			end
		end
		
		when slime_dungeon_10minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
		...
		when slime_dungeon_5minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				...
				server_timer("slime_dungeon_1minutesleft", 60, d.get_map_index())
			end
		end
		...
		when slime_dungeon_is_done.server_timer begin
			if d.select(get_server_timer_arg()) then
				...
				slime_dungeon.clear_slimedungeon()
				d.set_warp_location(164, 185, 185)
			end
