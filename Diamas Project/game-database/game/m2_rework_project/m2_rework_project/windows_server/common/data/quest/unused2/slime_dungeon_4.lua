quest slime_dungeon_4 begin
	state start begin
	--QUEST FUNCTIONS
		function settings()
			return
			{
				["slime_dung_index_4"] = 243,
		...
		function get_regens(level)
			local regens = {
				[1] = "data/dungeon/slime_cave4/regen_1.txt",
				[2] = "data/dungeon/slime_cave4/regen_2.txt",
				[3] = "data/dungeon/slime_cave4/regen_3.txt"};
		...
		function is_slimed_4()
			local pMapIndex = pc.get_map_index();
			local data = slime_dungeon_4.settings();
			local map_index = data["slime_dung_index_4"];
		...
		function check_enter()
			...
			say("")
			local settings = slime_dungeon_4.settings()
			...
			say_reward("Be fast!")
			wait()
			slime_dungeon_4.create_dungeon()
			d.add_boss_vnum(3768)
		end
				
		function create_dungeon()
			local settings = slime_dungeon_4.settings()
			if party.is_party() then
				d.new_jump_party(settings["slime_dung_index_4"], settings["slime_dung_warp"][1], settings["slime_dung_warp"][2])
				pc.setqf("dungeon_start_time", get_global_time())
			else
				d.new_jump(settings["slime_dung_index_4"], settings["slime_dung_warp"][1], settings["slime_dung_warp"][2])
				pc.setqf("dungeon_start_time", get_global_time())
			end

			d.setf("slimedung_level", 1)
			d.spawn_mob(settings["slime_metin"], settings["slime_metin_pos"][1], settings["slime_metin_pos"][2])
			server_timer("slime_dungeon_4_15minutesleft", 15*60, d.get_map_index())
		end
		--FUNCTIONS END

		--DUNGEON CHECK
		
		when 70007.use with pc.in_dungeon(243) begin
		...
		when login begin
			...
			if indx == settings["slime_dung_index_4"] then
		...
		--MAP ENTER
		--DUNGEON ENTER
		when 39236.chat."Anime-Dungeon (150+)"  with not slime_dungeon_4.is_slimed_4() begin
			local settings = slime_dungeon_4.settings()
			addimage(25, 10, "slime_dungeon.tga")
			addimage(230, 150, "thurang.tga")
			say("")
			...
			if (select ("Yes", "No") == 1) then
				slime_dungeon_4.check_enter()
			end
		end
		
		when kill with slime_dungeon_4.is_slimed_4() and not npc.is_pc() and npc.get_race() == 38430 and d.getf("slimedung_level") == 1 begin
			...
			timer("slime_wave_spawn_4", 1)
		end
		
		when 9237.chat."Open the stone" with slime_dungeon_4.is_slimed_4()  and d.getf("slimedung_level") == 3 begin
		...
		when kill with slime_dungeon_4.is_slimed_4() and not npc.is_pc() and npc.get_race() == 38430 and d.getf("slimedung_level") == 4 begin
			local settings = slime_dungeon_4.settings()
		...
		when 9238.take with item.get_vnum() == 30721 and slime_dungeon_4.is_slimed_4() and d.getf("slimedung_level") == 5 begin
			local settings = slime_dungeon_4.settings()
		...
		when 9237.take with item.get_vnum() == 30722 and slime_dungeon_4.is_slimed_4() and d.getf("slimedung_level") == 6 begin
			local settings = slime_dungeon_4.settings()
			pc.remove_item(settings["keys"][2], 1)
			npc.kill(9237)
			slime_dungeon_4.clear_slimedungeon()
		...
		when kill with slime_dungeon_4.is_slimed_4() and not npc.is_pc() and npc.get_race() == 3768 and d.getf("slimedung_level") == 8 begin
			slime_dungeon_4.clear_slimedungeon()
			...
			if math.mod(pc.getqf("boss_kills"), 1) == 0 then
				...
				notice_all("%s hat den %s bereits %s Mal gekillt (%s Sekunden)", pc.get_name(), c_mob_name(3768), pc.getqf("boss_kills"), get_global_time() - pc.getqf("dungeon_start_time"))
				--notice_all("%s benötigte %s Sekunden", pc.get_name(), get_global_time() - pc.getqf("dungeon_start_time"))
			end
			dungeon_info.set_ranking(DUNGEON_ID_SLIME_4, pc.getqf("boss_kills"), get_global_time() - pc.getqf("dungeon_start_time"), d.get_boss_max_dmg(npc.get_race()))
			pc.update_dungeon_progress(DUNGEON_ID_SLIME_4)
			d.completed()
			server_timer("slime_dungeon_4_is_done", 30, d.get_map_index())
			d.setf("slimedung_level", 9)
		end

		-- SPAWN OR NOTICE TIMER
		when slime_wave_spawn_4.timer begin
			local settings = slime_dungeon_4.settings()
			if d.getf("slimedung_level") == 2 then
				slime_dungeon_4.get_regens(1)
				loop_timer("slime_wave_kill", 5);
			elseif d.getf("slimedung_level") == 4 then
				d.setf("slime_stone", 1)
				slime_dungeon_4.get_regens(3)
		...
		when slime_dungeon_4_15minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				d.notice("Slime queen nest: !!!!! ONLY 10 MINUTES LEFT!!!!!")
				server_timer("slime_dungeon_4_10minutesleft", 10*60, d.get_map_index())
			end
		end
		
		when slime_dungeon_4_10minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				d.notice("Slime queen nest: !!!!! ONLY 5 MINUTES LEFT!!!!!")
				d.notice("Slime queen nest: You're running out of time!")
				server_timer("slime_dungeon_4_5minutesleft", 5*60, d.get_map_index())
			end
		end
		
		when slime_dungeon_4_5minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				d.notice("Slime queen nest: !!!!! ONLY 1 MINUTES LEFT!!!!!")
				d.notice("Slime queen nest: You are almost failed!")
				server_timer("slime_dungeon_4_1minutesleft", 60, d.get_map_index())
			end
		end
		
		when slime_dungeon_4_1minutesleft.server_timer begin
			if d.select(get_server_timer_arg()) then
				server_timer("slime_dungeon_4_is_done", 1, d.get_map_index())
			end
		end
		
		when slime_dungeon_4_is_done.server_timer begin
			if d.select(get_server_timer_arg()) then
