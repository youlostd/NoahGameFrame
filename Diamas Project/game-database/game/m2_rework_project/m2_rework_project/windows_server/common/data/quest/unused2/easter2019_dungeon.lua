quest easter2019_dungeon begin
	...
	function settings()
		return
		{
			["easter2019_dungeon_index"] = 231, ---- Dungeon index
			["easter2019_dungeon_index_out"] = 234, ---- Map index, where players will be teleported out of dungeon
			["easter2019_out_pos"] = {402,222}, ---- Coordinations, where players will be teleported out of dungeon
	...
	function check_enter()
		...
		if party.is_party() then			
			...
			--if not party.is_map_member_flag_lt("exit_easter2019_dungeon_time", get_global_time() - 1) then
	...
	function create_dungeon()
		...
		if party.is_party() then
			for i, pid in next, pids, nil do
				q.begin_other_pc_block(pid)
				pc.remove_item(settings["pass"], 1)
				q.end_other_pc_block()
			end
			d.new_jump_party(settings["easter2019_dungeon_index"], settings["dungeon_floor_pos"][1][1], settings["dungeon_floor_pos"][1][2])
			d.setf("easter2019_dungeon_level", 1)
			d.regen_file("data/dungeon/easter2019_dungeon/regen_1a.txt")
			d.spawn_mob_dir(settings["door"], settings["door_pos"][1][1], settings["door_pos"][1][2], settings["door_pos"][1][3])
			server_timer("easter2019_dungeon_30min_left", 30*60, d.get_map_index())
			server_loop_timer("easter2019_dungeon_wave_kill", 2, d.get_map_index())
		elseif (not settings["GroupType"]) then
				if settings["pass"] then
					pc.remove_item(settings["pass"], 1)
				end
			pc.remove_item(settings["pass"], 1)
			d.new_jump(settings["easter2019_dungeon_index"], settings["dungeon_floor_pos"][1][1]*100, settings["dungeon_floor_pos"][1][2]*100)
			d.setf("easter2019_dungeon_level", 1)
			d.regen_file("data/dungeon/easter2019_dungeon/regen_1a.txt")
			d.spawn_mob_dir(settings["door"], settings["door_pos"][1][1], settings["door_pos"][1][2], settings["door_pos"][1][3])
			server_timer("easter2019_dungeon_30min_left", 30*60, d.get_map_index())
		...
		when 70007.use with pc.in_dungeon(231) begin
		...
		when 9248.take with item.get_vnum() == 30732 and easter2019_dungeon.is_active() begin
			...
			elseif d.getf("easter2019_dungeon_level") == 12 then
				...
				d.regen_file("data/dungeon/easter2019_dungeon/regen_boss.txt")
		...
		when kill with easter2019_dungeon.is_active() and not npc.is_pc() and npc.get_race() == 8435 and d.getf("easter2019_dungeon_level") == 4 begin
			...
			if (d.getf("ed2019_egg1") < kills) then
				d.notice("Easter dungeon: %s eggs has left!", kills-d.getf("ed2019_egg1"))
			else
				d.notice("Easter dungeon: Well done! Now kill all monsters!")
				d.setf("easter2019_dungeon_level", 5)
				server_timer("easter2019_dungeon_spawner", 2, d.get_map_index())
			end 
		end 
		...
		when kill with easter2019_dungeon.is_active() and not npc.is_pc() and npc.get_race() == 8434 and d.getf("easter2019_dungeon_level") == 9 begin
			...
			if (d.getf("ed2019_metin1") < kills) then
				d.notice("Easter dungeon: %s metin stones has left!", kills-d.getf("ed2019_metin1"))
			else
				d.notice("Easter dungeon: Well done! Now kill all monsters!")
		...
		---------------------4. FLOOR
		when kill with easter2019_dungeon.is_active() and not npc.is_pc() and npc.get_race() == 487 or npc.get_race() == 488 and d.getf("easter2019_dungeon_level") == 13 begin
			local settings = easter2019_dungeon.settings()
			d.kill_all()
			...
			if math.mod(pc.getqf("boss_kills"), 5) == 0 then
				notice_all("%s hat den %s bereits %s Mal gekillt", pc.get_name(), c_mob_name(487), pc.getqf("boss_kills"))
			end
			--if party.is_party() then
			...
			--end
			server_timer("easter2019_dung_final_exit", 30, d.get_map_index())
		end
		...
		when easter2019_dung_final_exit.server_timer begin
			...
			server_timer("easter2019_dung_final_exit_out", 2, d.get_map_index())
		end
