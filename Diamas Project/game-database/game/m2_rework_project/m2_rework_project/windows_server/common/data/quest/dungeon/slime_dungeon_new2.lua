...
define PATH_TO_REGENFILES "data/dungeon/slime_dungeon_new_2/"
define EXIT_MAP_INDEX 163
define EXIT_MAP_X 934
define EXIT_MAP_Y 124

quest slime_dungeon_new_2 begin
	state start begin
		when 19236.chat."Dungeon!" with pc.get_map_index() == ENTER_MAP_INDEX begin
			enterDungeon(DUNGEON_ID_SLIME_2, DUNGEON_MAP_INDEX, LOCAL_BASE_X, LOCAL_BASE_Y, ONLY_GROUP_CAN_ENTER, EXIT_MAP_INDEX, EXIT_MAP_X, EXIT_MAP_Y)
		end
		...
		when login with pc.in_dungeon(DUNGEON_MAP_INDEX) and isDungeonLeader() and getDungeonStage() == 0 begin
			...
			d.spawn_mob(19237, 214, 269)
			d.spawn_mob(19237, 249, 295)
			d.spawn_mob(19237, 288, 266)
			d.spawn_mob(19237, 271, 229)
			d.spawn_mob(19237, 225, 222)
			...
			d.add_boss_vnum(17681)
			--d.notice("Stage 1: Kill the monster!")
			slime_dungeon_new_2.setDungeonTimeout(60 * 60)
		...
		when kill with pc.in_dungeon(DUNGEON_MAP_INDEX) begin
			...
			local isLastMonster = getMonsterCount() == getNeededMonsterCount()
				...
				--slime_dungeon_new_2.setTimeout(5 * 60)
				--increaseDungeonStage()
				--setNeededMonsterCount(106)
				end
			elseif stage == 2 and race == 1767 then
				setMonsterCount(getMonsterCount() + 1)
				...
				if isLastMonster then
                    game.drop_item(30721, 1)
                --end
					--slime_dungeon_new_2.clearTimeout()
					--d.purge()
					--slime_dungeon_new_2.jumpAll(658, 249)
					--slime_dungeon_new_2.setTimeout(5 * 60)
			...
			elseif stage == 3 and race == 18430 then
				setMonsterCount(getMonsterCount() + 1)

				if getMonsterCount() == getNeededMonsterCount() then
					--slime_dungeon_new_2.clearTimeout()

					d.spawn_mob(18430, 255, 265)
					increaseDungeonStage()
					setNeededMonsterCount(1)
				end
			elseif stage == 4 and race == 18430 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 5 and race == 1766 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 6 and race == 18430 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 7 and race == 1767 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 8 and race == 18430 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 9 and race == 1766 then
				setMonsterCount(getMonsterCount() + 1)
			...
			elseif stage == 10 and race == 1768 or race == 17681 then
				...
				dungeon_info.set_ranking(DUNGEON_ID_SLIME_2, pc.getqf("boss_kills"), get_global_time() - pc.getqf("dungeon_start_time"), d.get_boss_max_dmg(race))
...
--end	
	...
	when 19237.take with pc.in_dungeon(DUNGEON_MAP_INDEX) begin
			local stage = getDungeonStage()
