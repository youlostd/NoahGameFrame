quest demon_dungeon begin
	state start begin
		...
		function enter_dungeon()
			...
			server_timer("exit_almost_done_demon", 120 * 60, d.get_map_index())

			d.setf("group_timer", get_global_time(), 120 * 60)
...
say(string.format(gameforge.demon_dungeon._70_say, c_item_name(settings.ticket_vnum)))
		...
		when 271.kill or 272.kill or 273.kill begin
			local count_kill_for_key, count = 56, d.getf("count") + 1
			...
			if count == count_kill_for_key then
				game.drop_item(51072, 1)
				d.setf("count", 0)
				d.setf("stage_key_done", 1)
			end
		end
		
		when 51072.use with d.getf("stage") == 1 begin
			local settings = demon_dungeon.settings()
		...
		when 273.kill or 275.kill or 276.kill or 277.kill begin
			local count_kill_for_key2, count2 = 80, d.getf("count2") + 1
		...
		when 51072.use with d.getf("stage") == 2 begin
			local settings = demon_dungeon.settings()
		...
		when 277.kill or 279.kill begin
			local count_kill_for_key3, count3 = 90, d.getf("count3") + 1
		...
		when 51072.use with d.getf("stage") == 3 begin
			local settings = demon_dungeon.settings()
		...
		when 281.kill or 282.kill begin
			local count_kill_for_key4, count4 = 100, d.getf("count4") + 1
		...
		when 51072.use with d.getf("stage") == 4 begin
			local settings = demon_dungeon.settings()
		...
		when 283.kill with d.getf("stage") == 4 begin
			...
			cleartimer("demon_dungeon_exit_3", get_server_timer_arg())
			cleartimer("demon_dungeon_exit", get_server_timer_arg())
			timer("demon_dungeon_exit", 60)
			d.kill_all()
		...
		when logout with pc.in_dungeon() begin
			if pc.count_item(51072) >= 1 then
				pc.remove_item(51072, pc.count_item(51072))
			end
		end
		...
		when demon_dungeon_exit.timer begin
			...
			if index >= (13 * 10000) and index < (13+1 * 10000) then
