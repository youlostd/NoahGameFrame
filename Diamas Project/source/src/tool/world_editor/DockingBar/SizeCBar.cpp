...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    szRet.cx = std::max<long>(m_szMin.cx, szRet.cx);
    szRet.cy = std::max<long>(m_szMin.cy, szRet.cy);
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, uint32_t dwMode)
{
    ...
    if (m_bFloatFixedWidth)
        m_szFloat.cx = std::max<long>(0, m_szMin.cx);
    else
        m_szFloat.cx = std::max<long>(m_szFloat.cx, m_szMin.cx);

    m_szFloat.cy = std::max<long>(m_szFloat.cy, m_szMin.cy);
    return m_szFloat;
}
...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
    ...
    for (int i = 0; i < arrSCBars.GetSize(); i++)
        if (bHorz)
            m_szMinT.cy = std::max(m_szMinT.cy, arrSCBars[i]->m_szMin.cy);
        else
            m_szMinT.cx = std::max(m_szMinT.cx, arrSCBars[i]->m_szMin.cx);
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    // enforce the limits
    sizeNew.cx = std::max(m_szMinT.cx, std::min(m_szMaxT.cx, sizeNew.cx));
    sizeNew.cy = std::max(m_szMinT.cy, std::min(m_szMaxT.cy, sizeNew.cy));
...
BOOL CSizingControlBar::NegociateSpace(int nLengthAvail, BOOL bHorz)
{
    ...
    for (int iBar = 0; iBar < arrSCBars.GetSize(); iBar++)
    {
        ...
        nMinLength += bHorz ? pBar->m_szMin.cx - 2:
            pBar->m_szMin.cy - 2;
        nWidth = std::max<int>(nWidth, bHorz ? pBar->m_szHorz.cy :
            pBar->m_szVert.cx);
    }
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = std::max<long>(m_szMin.cx, pApp->GetProfileInt(szSection,
        _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = std::max<long>(m_szMin.cy, pApp->GetProfileInt(szSection,
        _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = std::max<long>(m_szMin.cx, pApp->GetProfileInt(szSection,
        _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = std::max<long>(m_szMin.cy, pApp->GetProfileInt(szSection,
        _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = std::max<long>(m_szMin.cx, pApp->GetProfileInt(szSection,
        _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = std::max<long>(m_szMin.cy, pApp->GetProfileInt(szSection,
