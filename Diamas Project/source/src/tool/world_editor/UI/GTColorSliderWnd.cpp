...
BOOL GTColorSliderWnd::Create(uint32_t dwStyle,CRect rcPos, CWnd* pParent,UINT nID,int nBulletType,BOOL bPopup)
{
	nBulletType;
	bPopup;
...
void GTColorSliderWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	if (row_buffer)
		delete [] row_buffer;
	row_buffer = new uint32_t [m_nBuffSize];
	if (row_buffer == NULL)
		return;
...
void GTColorSliderWnd::OnDraw(void)
{
	...
	uint32_t	*prgb;	// pointer to first pixel from the blend
	int		nSkip;	// number of pixels to skip after current row and before next one

	uint32_t	*p;		// loop pointers
	...
	{
		GTDrawHelper m_Panel;
		m_Panel.reverse((unsigned long*)row_buffer, m_nBuffSize);
	}
	...
	if (m_nCurLayout & layout_horizontal)
	{
		...
		{
			GTDrawHelper m_Panel;
			m_Panel.copy((unsigned long*)&p, (unsigned long*)row_buffer, blend_width), p += nSkip;
		}
	}
	else // if (m_nCurLayout & layout_vertical)
	{
		uint32_t	*source = row_buffer;

		i = blend_height;
		while (i--)
		{
			GTDrawHelper m_Panel;
			m_Panel.set((unsigned long*)&p, *source++, blend_width), p += nSkip;
		}
	}
}
