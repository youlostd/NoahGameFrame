...
void GTDrawHelper::GetRGB(uint32_t *buffer,int samples,COLORREF start,COLORREF end)
{
	int	red, green, blue;
...
void GTDrawHelper::HSV_HUE(uint32_t *buffer,int samples,double sat,double val_fp)
{
...
void GTDrawHelper::HSV_SAT(uint32_t *buffer,int samples,double hue,double val_fp)
{
	// value, but as integer in [0, 255 << int_extend]
	int		val;
...
void GTDrawHelper::HSV_VAL(uint32_t *buffer,int samples,double hue,double sat)
{
	// loop counter
	int		j;
