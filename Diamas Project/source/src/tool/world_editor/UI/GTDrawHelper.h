...
public:
	...
	// Get rgb value.
	void GetRGB(uint32_t *buffer,int samples,COLORREF start,COLORREF end);

	// hsv from hue.
	void HSV_HUE(uint32_t *buffer,int samples,double sat,double val);

	// hsv from sat.
	void HSV_SAT(uint32_t *buffer,int samples,double hue,double val);

	// hsv value.
	void HSV_VAL(uint32_t *buffer,int samples,double hue,double sat);
