...
BOOL GTColorSelectorWnd::Create(uint32_t dwStyle,CRect rcPos, CWnd* pParent,UINT nID,int nBulletType,BOOL bPopup)
{
	nBulletType;
	bPopup;
...
void GTColorSelectorWnd::OnPaint()
{
	...
	if (hdc != NULL)
	{
		...
		if (CONTRAST_MARKER)
		{
			...
			c = GetColor();
			val = std::max(std::max(GetRValue(c), GetGValue(c)), GetBValue(c));
			if (val < 128)
...
void GTColorSelectorWnd::DoDrawRGB(void)
{
	...
	uint32_t	*p;		// pointer to first pixel from the blend & a loop pointer
