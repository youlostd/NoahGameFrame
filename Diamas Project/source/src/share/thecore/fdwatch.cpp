...
LPFDWATCH fdwatch_new(int nfiles)
{
	int kq = kqueue();
	if (kq == -1) {
		spdlog::error( "kqueue() failed with {0}", errno);
		return NULL;
	}
...
int fdwatch(LPFDWATCH fdw, struct timeval* timeout)
{
	...
	if (!timeout) {
		...
		r = kevent(fdw->kq, fdw->kqevents, fdw->nkqevents, fdw->kqrevents,
		           fdw->nfiles, &ts);
	} else {
	...
	for (i = 0; i < r; i++) {
		...
		if (fd < fdw->nfiles) {
			...
			spdlog::error( "ident overflow {0} nfiles: {1}",
			          fdw->kqrevents[i].ident, fdw->nfiles);
		}
	}
...
void fdwatch_register(LPFDWATCH fdw, int flag, int fd, int rw)
{
	if (flag == EV_DELETE) {
		...
		fdw->kqevents[fdw->nkqevents].ident = fd;
		fdw->kqevents[fdw->nkqevents].flags = flag;
		fdw->kqevents[fdw->nkqevents].filter =
		    (rw == FDW_READ) ? EVFILT_READ : EVFILT_WRITE;
...
int fdwatch_check_event(LPFDWATCH fdw, socket_t fd, unsigned int event_idx)
{
	...
	if (fdw->kqrevents[event_idx].filter == EVFILT_READ) {
		...
		spdlog::error(
		          "fdwatch_check_event: Unknown filter %d (descriptor %d)",
...
void fdwatch_del_fd(LPFDWATCH fdw, socket_t fd)
{
	...
	std::move(&fdw->select_fds[idx + 1], &fdw->select_fds[fdw->nselect_fds],
	          &fdw->select_fds[idx]);

	std::move(&fdw->fd_data[idx + 1], &fdw->fd_data[fdw->nselect_fds],
	          &fdw->fd_data[idx]);

	std::move(&fdw->fd_rw[idx + 1], &fdw->fd_rw[fdw->nselect_fds],
	          &fdw->fd_rw[idx]);
...
int fdwatch(LPFDWATCH fdw, struct timeval* timeout)
{
	...
	if (!timeout) {
		tv.tv_sec = 0;
		tv.tv_usec = 0;
		r = select(0, &fdw->working_rfd_set, &fdw->working_wfd_set, (fd_set*)0,
		           &tv);
	} else {
		tv = *timeout;
		r = select(0, &fdw->working_rfd_set, &fdw->working_wfd_set, (fd_set*)0,
