...
*/
#ifndef METIN2_CLIENT_SPHERELIB_SPHEREPACK_HPP
#define METIN2_CLIENT_SPHERELIB_SPHEREPACK_HPP
...
{
    SPF_SUPERSPHERE =(1 << 0),
    // this is a supersphere, allocated and deleted by us
    SPF_ROOT_TREE =(1 << 1),
    // member of the root tree
    SPF_LEAF_TREE =(1 << 2),
    // member of the leaf node tree
    SPF_ROOTNODE =(1 << 3),
    // this is the root node
    SPF_RECOMPUTE =(1 << 4),
    // needs recomputed bounding sphere
    SPF_INTEGRATE =(1 << 5),
    // needs to be reintegrated into tree
...
public:
    ...
    virtual void VisibilityCallback(CGraphicObjectInstance *pInstance,
                                    // leaf node sphere in question
                                    ContainmentType state)    // new state it is in.
    {
    };
...
public:
    SpherePack(void)
    {
        ...
        mCenter = Vector3{0, 0, 0}; // default center position.
        IS_SPHERE = false;
    };
    ...
    {
        return Vector3::DistanceSquared(mCenter, pack->mCenter);
    };
