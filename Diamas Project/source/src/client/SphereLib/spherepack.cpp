                                          ...
                                          int flags)
{
    ...
    if (!pack)
    {
        SPDLOG_ERROR("mSpheres pool exhausted");
        return nullptr;
    }
                                  ...
                                  float node_size)
{
    ...
    while (search)
    {
        if (search->HasSpherePackFlag(SPF_SUPERSPHERE) && !search->HasSpherePackFlag(SPF_ROOTNODE) && search->
            GetChildCount())
        {
    ...
    else
    {
        ...
        case VS_INSIDE:
            if (!HasSpherePackFlag(SPF_INSIDE))
            {
                ...
                SetSpherePackFlag(SPF_INSIDE);
                //callback->VisibilityCallback(, state, state);
            }
            break;
        case VS_OUTSIDE:
            if (!HasSpherePackFlag(SPF_HIDDEN))
                           ...
                           ViewState state)
{
    if (state == VS_PARTIAL)
    {
        float d = Vector3::Distance(p, mCenter);
...
void SpherePackFactory::VisibilityCallback(CGraphicObjectInstance *pInstance, ContainmentType state)
{
#ifdef SPHERELIB_STRICT
	if (!sphere->IS_SPHERE)
