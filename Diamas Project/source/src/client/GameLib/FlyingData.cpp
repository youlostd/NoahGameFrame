...
CFlyingData::TFlyingAttachData::TFlyingAttachData()
	: iType(0)
	, iFlyType(0)
	, bHasTail(false)
	, dwTailColor(0)
...
void CFlyingData::SetBombEffect(const char* szEffectName)
{
	...
	{
		m_dwBombEffectID = ComputeCrc32(0, m_strBombEffectName.c_str(),m_strBombEffectName.size());
	}
	else
	{
		m_dwBombEffectID = 0;
...
//		m_fCollisionSphereRadius = 0.0f;
	...
	else if (!m_strBombEffectName.empty())
	{
		...
		CEffectManager::Instance().RegisterEffect(m_strBombEffectName.c_str(), &m_dwBombEffectID);
	}
	...
	for(i=0;i<TextFileLoader.GetChildNodeCount();i++)
	{
		...
		if (strNodeName=="attachdata")
		{
			...
			if (temp)
			{
				fad.bHasTail = true;
				std::string t;
				TextFileLoader.GetTokenString("tailcolor", &t);

				storm::ParseNumber(t, fad.dwTailColor);
		...
		{
			SPDLOG_ERROR("CFlyingData::LoadScriptFile Wrong Data : {1}", m_strFilename.c_str());
		}
	}
