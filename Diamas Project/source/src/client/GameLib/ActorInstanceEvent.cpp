...
void CActorInstance::__OnMoving()
{
    ...
    {
        kPPosDir.Normalize();
        kPPosDir = XMVectorScale(kPPosDir, 1000.0f);
        kState.kPPosSelf = XMVectorAdd(kPPosDir, c_rkPPosCur);
    }
    else
    {
...
void CActorInstance::__OnUseSkill(UINT uMotSkill, UINT uLoopCount, bool isMovingSkill, uint32_t color)
{
    IEventHandler &rkEventHandler = __GetEventHandlerRef();
    ...
    rkEventHandler.OnUseSkill(kState, uMotSkill, uLoopCount, isMovingSkill, color);
}

void CActorInstance::__OnHit(uint32_t motionKey, UINT uSkill,
    CActorInstance& rkInstVictm, const Vector3& pushDest, bool isSendPacket)
{
     IEventHandler &rkEventHandler = __GetEventHandlerRef();
    rkEventHandler.OnHit(motionKey, uSkill, rkInstVictm, pushDest, isSendPacket);
}
...
CActorInstance::IEventHandler *CActorInstance::IEventHandler::GetEmptyPtr()
{
    ...
    public:
        ...
        virtual void OnHit(uint32_t motionKey, UINT uMotAttack, CActorInstance &rkActorVictim, const Vector3& pushDest, bool isSendPacket)
        {
        }
