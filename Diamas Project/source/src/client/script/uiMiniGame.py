    ...
    class MiniGameDialog(ui.ScriptWindow):
        ...
        def __LoadWindow(self):
            ...
            try:
                ...
                import logging
                logging.exception("MiniGameDialog.LoadWindow.LoadObject")
            ...
            try:
                ...
                import logging
                logging.exception("MiniGameDialog.LoadWindow.BindObject")

            self.Hide()

        def AppendButton(self, name, func):

            if name in self.button_dict:
                return
...
class MiniGameWindow(ui.ScriptWindow):
    ...
    def __LoadWindow(self):
        ...
        try:
            ...
            logging.exception("MiniGameWindow.LoadWindow.LoadObject")

        try:
            self.minigame_rumi_button = self.GetChild("minigame_rumi_button")
            self.minigame_rumi_button.SetEvent(self.__ClickRumiButton)
            ...
            logging.exception("MiniGameWindow.LoadWindow.Okey.BindObject")
        ...
        if app.ENABLE_MONSTER_BACK or app.ENABLE_CARNIVAL2016 or app.ENABLE_MINI_GAME_OKEY_NORMAL or app.ENABLE_FISH_EVENT:
            try:
                ...
                self.event_banner_button.SetEvent(self.__ClickIntegrationEventBannerButton)
                self.event_banner_button.Hide()
                self.event_banner_button_enable = False

            except:

                logging.exception("MiniGameWindow.LoadWindow.EventBannerButton.BindObject")
            ...
            try:
                ...
                logging.exception("MiniGameWindow.LoadWindow.MiniGameDialog")
        ...
        def IntegrationMiniGame(self, enable):
            ...
            if self.mini_game_dialog:
                ...
                if False == enable:
                    ...
                    if app.ENABLE_MONSTER_BACK or app.ENABLE_CARNIVAL2016:
                        if player.GetAttendance():
                            self.mini_game_dialog.AppendButton(uiScriptLocale.BANNER_ATTENDANCE_BUTTON,
                                                               self.__ClickAttendanceButton)
                    if app.ENABLE_MONSTER_BACK and app.ENABLE_ACCUMULATE_DAMAGE_DISPLAY:
                        if player.GetMonsterBackEvent():
                            self.mini_game_dialog.AppendButton(uiScriptLocale.BANNER_MONSTERBACK_BUTTON,
                                                               self.__ClickMonsterBackButton)
                    if app.ENABLE_MINI_GAME_OKEY_NORMAL:
                        if player.GetMiniGameWindowOpen():
                            self.mini_game_dialog.AppendButton(uiScriptLocale.BANNER_OKEY_BUTTON,
                                                               self.__ClickRumiButton)
                    if app.ENABLE_FISH_EVENT:
                        if player.GetFishEventGame():
                            self.mini_game_dialog.AppendButton(uiScriptLocale.BANNER_FISH_BUTTON,
                                                               self.__ClickFishEventButton)
                    if app.ENABLE_MINI_GAME_YUTNORI:
                        if player.GetYutnoriGame():
                            self.mini_game_dialog.AppendButton(uiScriptLocale.BANNER_YUTNORI_BUTTON,
                                                               self.__ClickYutnoriButton)
    ...
    if app.ENABLE_MONSTER_BACK:
        def __ClickMonsterBackButton(self):
            ...
            ##appInst.instance().GetNet().SendRequestEventQuest("e_monsterback")
            self.main_game = None
            self.game_type = MINIGAME_TYPE_MAX
