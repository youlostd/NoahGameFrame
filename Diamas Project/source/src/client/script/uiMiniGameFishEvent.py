...
class FishEventGameWaitingPage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("FishEventGameWaitingPage.LoadWindow.LoadObject")
		...
		try:
			...
			logging.exception("FishEventGameWaitingPage.LoadWindow.BindObject")
...
class FishEventGamePage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("FishEventGamePage.LoadWindow.LoadObject")
		...
		try:
			...
			logging.exception("FishEventGamePage.LoadWindow.BindObject")
	...
	def OnTop(self):
		...
		self.interface.SetOnTopWindow(player.ON_TOP_WND_FISH_EVENT)
		self.interface.RefreshMarkInventoryBag()
	...
	def Show(self):
		...
		if 0 == self.use_count:
			appInst.instance().GetNet().SendRequestFishEventBlock()
		
		mouseModule.mouseController.DeattachObjectPostProcess()	
		## �κ��丮���� ITEM_FISH_EVENT_BOX, ITEM_FISH_EVENT_BOX_SPECIAL �� ã�´�.
		## �����Ѵٸ� slot �� ��� �����ش�.
		special_item_pos	= player.GetFishEventItemPos(ITEM_FISH_EVENT_BOX_SPECIAL)
		self.__AddFishEventSlot(SPECIAL_TYPE, special_item_pos)
		normal_item_pos		= player.GetFishEventItemPos(ITEM_FISH_EVENT_BOX)
	...
	def __AddAccept(self, slotIndex):
		...
		appInst.instance().GetNet().SendAddFishBox( slotIndex, self.shape)
		self.shape = 0
		self.DeattachObject()
	...
	def __UseAccept(self, type):
		self.__QuestionCancel()
		if self.fish_box_slot_pos[type] != -1:
			appInst.instance().GetNet().SendUseFishBox( player.SLOT_TYPE_INVENTORY, self.fish_box_slot_pos[type] )
	...
	def __ScoreTextEffectEndFrameEvent(self):
		...
		self.popup.SetText( localeInfo.MINIGAME_FISH_EVENT_REWARD_MSG.format(item_name))
		self.popup.Open()
		self.reward_vnum = 0
...
class MiniGameFish(ui.Window):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("MiniGameFish.LoadWindow")
