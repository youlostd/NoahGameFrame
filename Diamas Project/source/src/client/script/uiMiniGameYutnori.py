...
YUTNORI_STATE_THROW				= 0		# ���� ������ �ϴ� ����
YUTNORI_STATE_RE_THROW			= 1		# ��,�� ���� �ٽ� ������ ����
YUTNORI_STATE_MOVE				= 2		# ������ �̵��ؾ� �ϴ� ����
...
class YutnoriWaitingPage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("MiniGameYutnoriWaitingPage.LoadWindow.LoadObject")
		...
		try:
			...
			logging.exception("MiniGameYutnoriWaitingPage.LoadWindow.BindObject")
	...
	def __ClickStartButton(self):
		...
		if None == self.start_question_dialog:
			...
			self.start_question_dialog.SetText(localeInfo.MINI_GAME_YUTNORI_START_QUESTION.format(30000, 1) )
	...
	def __StartAccept(self):
		appInst.instance().GetNet().SendMiniGameYutnoriStart()
	...
	def Show(self):
		...
		event.SetRestrictedCount(self.descIndex, 90) #DESC_WIDTH_COUNT mainline �� development �� ��Ʈü�� �޶�
...
class YutArea:
	def __init__(self, parent, func, x, y, index, prev, next, shortcut_next):
		...
		if localeInfo.IsARABIC():
			# BOARD_WINDOW_WIDTH 436 - �����̹��� width 312 = 124
			self.pos			= (x + 124, y)
...
class YutnoriGamePage(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("YutnoriGame.LoadWindow.LoadObject")
		...
		try:
			...
			logging.exception("YutnoriGame.LoadWindow.__BindObject")
		...
		try:	
			...
			logging.exception("YutnoriGame.LoadWindow.__BindEvent")
	...
	def __GiveupAccept(self):
		...
		if False == self.is_actionable:
			return
		appInst.instance().GetNet().SendMiniGameYutnoriGiveup()
		self.__GiveupCancel()
	...
	def __ClickChar(self, event_type, index):		
		...
		if -1 == available_index:
			appInst.instance().GetNet().SendMiniGameCharClick( self.player_index )
		else:
	...
	def __ClickArea(self, event_type, index):
		...
		appInst.instance().GetNet().SendMiniGameYutMove( self.player_index )
		self.player_index = -1
	...
	def __ClickProbButton(self, index):
		...
		self.__ProbSelectWindowOpen( False )
		appInst.instance().GetNet().SendMiniGameYutnoriProb( index )
	...
	def __ClickRewardButton(self):
		appInst.instance().GetNet().SendMiniGameYutReward()
	...
	def __ClickThrowButton(self):
		...
		appInst.instance().GetNet().SendMiniGameYutThrow( True )
	...
	def __UpdateAlpha(self):
		...
		if self.yut_cur_alpha >= 1.0:
			...
			if self.yutnori_state in [YUTNORI_BEFORE_TURN_SELECT, YUTNORI_AFTER_TURN_SELECT]:
				...
				if self.notice_text:
					self.notice_text.SetText( localeInfo.MINI_GAME_NOTICE_8.format(prob_name_tuple[ yut_result ]) )	# %s ���Խ��ϴ�!
							
			elif True == self.pc_turn:
				move_count	= self.__GetMoveCount( self.yut_result_pc )
				notice_str	= None
				if move_count > 0:
					notice_str = localeInfo.MINI_GAME_NOTICE_6.format(prob_name_tuple[ self.yut_result_pc ], move_count )	# %s ���Խ��ϴ�! %dĭ �̵��� �� �ֽ��ϴ�.
				else:
					notice_str = localeInfo.MINI_GAME_NOTICE_7.format(prob_name_tuple[ self.yut_result_pc ])					# %s ���Խ��ϴ�! 1ĭ �ڷ� �̵� �ؾ��մϴ�.
	...
	def __UpdateEvent(self):
		...
		if len(self.event_deque) > 0:
			...
			if EVENT_TYPE_NOTICE == event_type:
				...
				self.event_deque.popleft()
				appInst.instance().GetNet().SendMiniGameYutThrow( False )
				...
				self.event_deque.popleft()
				appInst.instance().GetNet().SendMiniGameRequestComAction()
	...
	def __CatchCheck(self):
		...
		while len(self.catch_deque) > 0:
			...
			if is_pc:
				...
				notice_str = localeInfo.MINI_GAME_NOTICE_11							# COM ������ PC ��⿡ �����߽��ϴ�.
				...
				notice_str = localeInfo.MINI_GAME_NOTICE_10							# PC ������ COM ��⿡ �����߽��ϴ�.
	...
	def __TurnCheck(self):
		...
		if True == self.next_turn:
			if YUTNORI_AFTER_TURN_SELECT == self.yutnori_state:
				...
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_5] )		# PC�� ������ �� ���� PC ���� �̵��մϴ�.
				...
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_2] )			# �� �����⸦ ���� ������ �� �ֽ��ϴ�.
				self.event_deque.append( [EVENT_TYPE_CHANGE_TEXT_COLOR, True] )							# text color pc turn ���� ����
				...
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_12] )			# �� �����⸦ �ѹ� �� �� �� �ֽ��ϴ�.
				self.event_deque.append( [EVENT_TYPE_CHANGE_TEXT_COLOR, True] )							# text color pc turn ���� ����
				self.event_deque.append( [EVENT_TYPE_BUTTON_FLASH, (self.yut_throw_button, True)] )		# Yut Throw button Enalbe Flush
		...
		## ���� ���� COM ���̴�.
			...
			if YUTNORI_AFTER_TURN_SELECT == self.yutnori_state:
				...
				self.event_deque.append( [EVENT_TYPE_INSER_DELAY, 1000] )
				self.event_deque.append( [EVENT_TYPE_BUTTON_FLASH, (self.yut_throw_button, False)] )	# Yut Throw button Enalbe Flush
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_4] )			# COM�� ������ �� ���� COM ���� �̵��մϴ�.
				self.event_deque.append( [EVENT_TYPE_INSER_DELAY, 2000] )
				self.event_deque.append( [EVENT_TYPE_CALL_TURN_CHECK, None] )
			elif YUTNORI_BEFORE_TURN_SELECT == self.yutnori_state:
				self.event_deque.append( [EVENT_TYPE_INSER_DELAY, 2000] )
				self.event_deque.append( [EVENT_TYPE_BUTTON_FLASH, (self.yut_throw_button, False)] )	# Yut Throw button Enalbe Flush
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_3] )			# �� ������ ���� COM�� �������Ⱑ ����˴ϴ�.
				self.event_deque.append( [EVENT_TYPE_CHANGE_TEXT_COLOR, False] )						# text color com turn ���� ����
				self.event_deque.append( [EVENT_TYPE_COM_YUT_THROW, None] )								# ��ǻ�Ͱ� ���� �������� ��û.
			elif self.yutnori_state in [YUTNORI_STATE_THROW, YUTNORI_STATE_RE_THROW]:
				self.event_deque.append( [EVENT_TYPE_BUTTON_FLASH, (self.yut_throw_button, False)] )	# Yut Throw button Enalbe Flush
				self.event_deque.append( [EVENT_TYPE_NOTICE, localeInfo.MINI_GAME_NOTICE_9] )			# COM ���� �� �Դϴ�. ��ø� ��ٷ��ּ���
				self.event_deque.append( [EVENT_TYPE_CHANGE_TEXT_COLOR, False] )						# text color com turn ���� ����
				self.event_deque.append( [EVENT_TYPE_COM_YUT_THROW, None] )								# ��ǻ�Ͱ� ���� �������� ��û.
			elif YUTNORI_STATE_MOVE == self.yutnori_state:
				self.event_deque.append( [EVENT_TYPE_BUTTON_FLASH, (self.yut_throw_button, False)] )	# Yut Throw button Enalbe Flush
				self.event_deque.append( [EVENT_TYPE_SHOW_UNIT, False] )
				self.event_deque.append( [EVENT_TYPE_REQUEST_COM_ACTION, None] )						## COM ���� �������� ��û�Ѵ�.
	...
	def __PlayerWinPopup(self):
		...
		self.pc_win_popup.SetText2( localeInfo.MINI_GAME_YUTNORI_PC_WIN_SCORE.format(self.score))
...
class MiniGameYutnori(ui.Window):
	...
	def __LoadWindow(self):
		...
		try:
			...
			logging.exception("MiniGameYutnori.LoadWindow")
