...
bool CPythonNetworkDatagram::CheckPacket(TPacketHeader * piRetHeader)
{
	...
	*piRetHeader = 0;
	...
	{
		SPDLOG_DEBUG("Unknown UDP packet header");
...
void CPythonNetworkDatagram::SetConnection(const char * c_szIP, uint16_t wPortIndex)
{
	m_NetSender.SetSocket(c_szIP, wPortIndex);
}

void CPythonNetworkDatagram::SetRecvBufferSize(uint32_t dwSize)
{
	m_NetReceiver.SetRecvBufferSize(dwSize);
}

void CPythonNetworkDatagram::SendToServer(const void * c_pBuffer, uint32_t dwSize)
{
...
void CPythonNetworkDatagram::Bind(uint32_t dwAddress, uint16_t wPortIndex)
{
	m_NetReceiver.Bind(dwAddress, wPortIndex);
}

void CPythonNetworkDatagram::RegisterSender(uint32_t dwID, uint32_t dwAddress, uint16_t wPortIndex)
{
...
void CPythonNetworkDatagram::DeleteSender(uint32_t dwID)
{
...
void CPythonNetworkDatagram::Select(uint32_t dwID)
{
	CNetDatagramSender * pSender;
	if (!GetSenderPointer(dwID, &pSender))
		return;
...
void CPythonNetworkDatagram::SendToSenders(const void * c_pBuffer, uint32_t dwSize)
{
	// NOTE : Temporary Code
...
bool CPythonNetworkDatagram::GetSenderPointer(uint32_t dwID, CNetDatagramSender ** ppSender)
{
...
void CPythonNetworkDatagram::SendCharacterStatePacket(uint32_t dwVID, uint32_t dwCmdTime, const TPixelPosition& c_rkPPosDst, float fDstRot, UINT eFunc, UINT uArg)
{	
...
bool CPythonNetworkDatagram::RecvStateWalkingPacket()
{
	...
	if (!m_NetReceiver.Recv(&kStatePacket, sizeof(kStatePacket)))
	{
		...
		SPDLOG_DEBUG("CPythonNetworkDatagram::RecvStatePacket - PAKCET READ ERROR");
		return FALSE;
	}
	...
	if (!pkChrInst)
	{
		//SPDLOG_DEBUG("CPythonNetworkDatagram::RecvStatePacket - NOT EXIST VID(kStateWaitingPacket.vid = %d)", kStatePacket.m_dwVID);
		return TRUE;
	}
