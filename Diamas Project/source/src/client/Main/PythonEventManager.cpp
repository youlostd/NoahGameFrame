...
void GetCameraSettingFromArgList(ScriptGroup::TArgList &rArgList, CPythonApplication::SCameraSetting *pCameraSetting)
{
    ...
    ZeroMemory(pCameraSetting, sizeof(CPythonApplication::SCameraSetting));
...
void CPythonEventManager::__InitEventSet(TEventSet &rEventSet)
{
    ...
    rEventSet.CurrentColor = DirectX::SimpleMath::Color(1, 1, 1, 1);
    rEventSet.strCurrentLine = "";

    rEventSet.currentLine.instance.reset();
...
int CPythonEventManager::RegisterEventSet(const std::string &c_szFileName)
{
    auto str = LoadFileToString(GetVfs(), c_szFileName.c_str());
    if (!str)
        return -1;
    ...
    if (!pEventSet->ScriptGroup.Create(str.value()))
    {
        __ClearEventSetp(pEventSet);
        return -1;
    }

    // pEventSet->filename = c_szFileName;
...
int CPythonEventManager::RegisterEventSetFromString(const std::string &strScript, bool bIsQuestInfo)
{
    TEventSet *pEventSet = m_EventSetPool.Alloc();
    if (!pEventSet)
        return -1;
...
void CPythonEventManager::__ClearEventSetp(TEventSet *pEventSet)
{
    ...
    pEventSet->currentLine.instance.reset();
...
void CPythonEventManager::SetRestrictedCount(int iIndex, int iCount)
{
    ...
    if (!pEventSet)
    {
        SPDLOG_ERROR("m_EventSetVector[iIndex={0}]==NULL", iIndex);
        return;
    }
...
void CPythonEventManager::InsertText(int iIndex, const char *c_szText, int iX_pos)
{
    ...
    if (pEventSet->currentLine.instance)
        pEventSet->currentLine.instance->SetValue(c_szText);
...
void CPythonEventManager::UpdateEventSet(int iIndex, int ix, int iy)
{
    ...
    {
        int iLeftTime = std::max<int>(0, pEventSet->iConfirmEndTime - ELTimer_GetMSec() / 1000);
        pEventSet->pConfirmTimeTextLine->SetValue(fmt::format(m_strLeftTimeString.c_str(), iLeftTime));
    }
    ...
    // Process EventSet
    auto lElapsedTime = DX::StepTimer::instance().GetElapsedMillieSeconds();

    pEventSet->lLastDelayTime = std::max<uint64_t>(0, pEventSet->lLastDelayTime - lElapsedTime);
...
void CPythonEventManager::SetFontColor(int iIndex, float r, float g, float b)
{
    ...
    pEventSet->CurrentColor = DirectX::SimpleMath::Color(r, g, b, 0.7f);

    pEventSet->currentLine.instance->SetColor(pEventSet->CurrentColor.BGRA().c);
    pEventSet->currentLine.instance->Update();

    for (const auto &textLine : pEventSet->ScriptTextLineList)
    {
        textLine.instance->SetColor(pEventSet->CurrentColor.BGRA().c);
        textLine.instance->Update();
    }
}
...
void CPythonEventManager::ProcessEventSet(TEventSet *pEventSet)
{
    ...
    CPythonApplication &rApp = CPythonApplication::AppInst();
    ...
    case EVENT_TYPE_LETTER: {
        const auto &c_rstValue = GetArgumentString("value", ScriptCommand.argList);
        pEventSet->strCurrentLine.append(c_rstValue);
        pEventSet->currentLine.instance->SetValue(pEventSet->strCurrentLine);
    ...
    case EVENT_TYPE_COLOR: {
        ...
        {
            pEventSet->CurrentColor = DirectX::SimpleMath::Color(1.0f, 1.0, 1.0f, 1.0f);
        }
        break;
    }
    ...
    case EVENT_TYPE_NEXT: {
		pEventSet->qIndex = atoi(GetArgument("resume", ScriptCommand.argList));
    ...
    case EVENT_TYPE_LEFT_IMAGE: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnLeftImage",
                              Py_BuildValue("(s)", GetArgument("src", ScriptCommand.argList)));
        break;
    }

    case EVENT_TYPE_TOP_IMAGE: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnTopImage",
                              Py_BuildValue("(s)", GetArgument("src", ScriptCommand.argList)));
        break;
    }

    case EVENT_TYPE_BACKGROUND_IMAGE: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnBackgroundImage",
                              Py_BuildValue("(s)", GetArgument("src", ScriptCommand.argList)));
        break;
    }
    ...
    case EVENT_TYPE_VIDEO: {
        ...
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnVideo", Py_BuildValue("(iis)", x, y, src));
        break;
    }
    ...
    case EVENT_TYPE_INSERT_IMAGE: {
        ...
        if (imageFile.empty())
        {
            ...
            PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInsertItemIcon",
                                  Py_BuildValue("(sissii)", imageType, iItemIndex, title, desc, index, total));
        }
        else
        {
            PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInsertImage",
                                  Py_BuildValue("(ssssii)", imageFile.c_str(), title, title, desc, index, total));
        }
        pEventSet->iAdjustLine += 2;
        break;
    }
    ...
    case EVENT_TYPE_SET_CENTER_MAP_POSITION: {
        CPythonMiniMap::Instance().SetAtlasCenterPosition(atoi(GetArgument("x", ScriptCommand.argList)),
    ...
    case EVENT_TYPE_FADE_IN: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "FadeIn",
                              Py_BuildValue("(f)", atof(GetArgument("speed", ScriptCommand.argList))));
        pEventSet->isWaitFlag = true;
        break;
    }
    case EVENT_TYPE_WHITE_OUT: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "WhiteOut",
                              Py_BuildValue("(f)", atof(GetArgument("speed", ScriptCommand.argList))));
        pEventSet->isWaitFlag = true;
        break;
    }
    case EVENT_TYPE_WHITE_IN: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "WhiteIn",
                              Py_BuildValue("(f)", atof(GetArgument("speed", ScriptCommand.argList))));
        pEventSet->isWaitFlag = true;
        break;
    }
    ...
    case EVENT_TYPE_TITLE_IMAGE: {
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnTitleImage",
                              Py_BuildValue("(s)", GetArgument("src", ScriptCommand.argList)));
        break;
    }
    case EVENT_TYPE_DUNGEON_RESULT: {
        ...
        PyCallClassMemberFunc(m_poInterface, "ShowDungeonResult",
                              Py_BuildValue("(iiiiiiiii)", killstone_count, killmob_count, find_hidden, hidden_total,
    ...
    case EVENT_TYPE_ITEM_NAME: {
        ...
        if (CItemManager::Instance().GetItemDataPointer(iIndex, &pItemData))
        {
            ...
            pEventSet->currentLine.instance->SetValue(pEventSet->strCurrentLine);
            pEventSet->currentLine.instance->SetColor(1.0f, 0.2f, 0.2f);
    ...
    case EVENT_TYPE_MONSTER_NAME: {
        ...
        auto optName = rkNonPlayer.GetName(iIndex);
    ...
    case EVENT_TYPE_INPUT: {
        int qIndex = atoi(GetArgument("resume", ScriptCommand.argList));

        __InsertLine(*pEventSet);
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInput", Py_BuildValue("(i)", qIndex));
        break;
    }
    case EVENT_TYPE_LONG_INPUT: {
        __InsertLine(*pEventSet);
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInputLong", Py_BuildValue("()"));
        break;
    }
    case EVENT_TYPE_CONFIRM_WAIT: {
        ...
        pEventSet->pConfirmTimeTextLine = pEventSet->currentLine.instance.get();
        pEventSet->iConfirmEndTime = ELTimer_GetMSec() / 1000 + iTimeOut;
    ...
    case EVENT_TYPE_INSERT_IMAGE_TOOLTIP: {
        int vnum = atoi(GetArgument("vnum", ScriptCommand.argList));
        int index = atoi(GetArgument("index", ScriptCommand.argList));
        int total = atoi(GetArgument("total", ScriptCommand.argList));

        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInsertImageShowItemToolTip",
                              Py_BuildValue("(iii)", vnum, index, total));
        pEventSet->iAdjustLine += 2;
        break;
    }
    case EVENT_TYPE_INPUT_ADD_CANCEL: {
        int qIndex = atoi(GetArgument("resume", ScriptCommand.argList));

        __InsertLine(*pEventSet);
        PyCallClassMemberFunc(pEventSet->poEventHandler, "OnInputAddcancel", Py_BuildValue("(i)", qIndex));
        break;
    }
    }
}
...
void CPythonEventManager::RenderEventSet(int iIndex)
{
    ...
    for (auto itor = pEventSet->ScriptTextLineList.begin(); itor != pEventSet->ScriptTextLineList.end();
         ++itor, ++iCount)
    {
    ...
    if (iCount >= pEventSet->iVisibleStartLine && iCount < pEventSet->iVisibleStartLine + pEventSet->iVisibleLineCount)
    {
...
void CPythonEventManager::MakeQuestion(TEventSet *pEventSet, ScriptGroup::TArgList &rArgumentList)
{
	...
	PyCallClassMemberFunc(pEventSet->poEventHandler, "MakeQuestion", Py_BuildValue("(i)", rArgumentList.size() - 1));

	int iIndex = 0;
	for (auto itor = rArgumentList.begin(); itor!=rArgumentList.end(); ++itor)
	{
		auto& rArgument = *itor;
		if (rArgument.strName == "resume") {
			pEventSet->qIndex = atoi(rArgument.strValue.c_str());
			continue;
		}
...
void CPythonEventManager::SelectAnswer(int iIndex, int iAnswer)
{
    ...
    gPythonNetworkStream->SendScriptAnswerPacket(iAnswer, pEventSet->qIndex);
}
...
{
    if (rEventSet.currentLine.instance)
    {
    ...
    if (!pkDefaultFont)
    {
        SPDLOG_ERROR("CPythonEventManager::InsertLine - CANNOT_FIND_DEFAULT_FONT");
        return;
    }

    rEventSet.currentLine.instance.reset(new CGraphicTextInstance());
    if (!rEventSet.currentLine.instance)
    {
        SPDLOG_ERROR("CPythonEventManager::InsertLine - OUT_OF_TEXT_LINE");
        return;
    }

    rEventSet.currentLine.instance->SetTextPointer(pkDefaultFont);
    rEventSet.currentLine.instance->SetColor(1.0f, 1.0f, 1.0f);
    rEventSet.currentLine.instance->SetValue("");
...
void CPythonEventManager::RenderTextLine(TEventSet &set, TTextLine &line)
{
    ...
    if (line.centered)
    {
        auto localX = line.ixLocal;
        if (0 == localX)
            localX = set.iWidth;

        x += (localX - line.instance->GetWidth()) / 2;
    }
    else
    {
        x += line.ixLocal;
    ...
    line.instance->Render(x, y);
}

void CPythonEventManager::__AddSpace(TEventSet &rEventSet, int iSpace) { rEventSet.iyLocal += iSpace; }
...
bool CPythonEventManager::GetScriptEventIndex(const char *c_szName, int *pEventPosition, int *pEventType)
{
    ...
    {
        SPDLOG_ERROR("Event name {0} not found", c_szEventName);
        return false;
    }
...
void CPythonEventManager::SetInterfaceWindow(PyObject *poInterface, PyObject *from)
{
    if (!from || m_poInterface == from)
        m_poInterface = poInterface;
}

void CPythonEventManager::SetLeftTimeString(const char *c_szString) { m_strLeftTimeString = c_szString; }

CPythonEventManager::CPythonEventManager() : m_poInterface(0), m_strLeftTimeString("남은 시간 : %d초")
{
    ...
    EventTypeMap["VIDEO"] = EVENT_TYPE_VIDEO;
    ...
    EventTypeMap["LONGINPUT"] = EVENT_TYPE_LONG_INPUT;
    ...
    EventTypeMap["INSERT_IMAGE_SHOWTOOLTIP"] = EVENT_TYPE_INSERT_IMAGE_TOOLTIP;
    EventTypeMap["INPUTADDCANCEL"] = EVENT_TYPE_INPUT_ADD_CANCEL;
}
