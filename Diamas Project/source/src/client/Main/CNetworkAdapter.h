...
uint32_t EnumNetworkAdapters(CNetworkAdapter *lpBuffer, ULONG ulSzBuf, LPDWORD lpdwOutSzBuf);
...
private:
    ...
    uint32_t m_dwIndex;   // machine index of the adapter.
    UINT m_nAdapterType;
    bool m_bDhcpUsed;
    bool m_bWinsUsed;
    ...
    struct UNNAMED
    {
        uint8_t ucAddress[MAX_ADAPTER_ADDRESS_LENGTH];
        UINT nLen;
    } m_ucAddress;
