...
void CInstanceBase::SetRank(uint8_t rank)
{
    m_GraphicThingInstance.SetRank(rank);
}

void CInstanceBase::SetScale(float fScale)
{
    m_GraphicThingInstance.SetScale(fScale, fScale, fScale);
    m_GraphicThingInstance.SetReachScale(fScale);
}
...
void CInstanceBase::NEW_LookAtDestInstance(CInstanceBase &rkInstDst)
{
    ...
    // 	SPDLOG_DEBUG("LookAt %f", m_GraphicThingInstance.GetTargetRotation());
}
