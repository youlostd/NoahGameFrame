...
// 라이브러리 초기화
HRESULT CBandiVideoLibrary::Create(LPCTSTR dll_path_name, IBandiVideoFileReader *reader /*= NULL */,
                                   IBandiVideoSound *sound /*= NULL */)
{
    if (m_dll)
    {
        ASSERT(0);
        return BVERR_ALREADY_CREATED;
    }
...
HRESULT CBandiVideoLibrary::Verify(LPCSTR id, LPCSTR key)
{
    ...
    {
        ASSERT(0);
        return BVERR_LIBRARY_NOT_LOADED;
    }
