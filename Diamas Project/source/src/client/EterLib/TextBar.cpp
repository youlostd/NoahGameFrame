...
void CTextBar::SetTextAlign (float fMode)
{
	HDC hDC = m_dib.GetDCHandle();
...
void CTextBar::DrawText (RECT& c_rRect, const char* c_szText)
{
	HDC hDC = m_dib.GetDCHandle();

	::DrawText (hDC, c_szText, ::strlen (c_szText), &c_rRect, DT_LEFT | DT_EXTERNALLEADING | DT_WORDBREAK);
	Invalidate();
}
...
CTextBar::CTextBar (int fontSize, bool isBold): m_hFont (nullptr)
{
	m_hOldFont = NULL;
	m_fontSize = fontSize;
	m_isBold = isBold;
