	...
	public:
		...
		void Push(const char * c_szName)
		{
			...
			if (m_GraphicTextInstanceMap.end() == itor)
			{
				...
				CResource * pResource = CResourceManager::Instance().LoadResource("����ü.fnt");
				pGraphicTextInstance->Clear();
				pGraphicTextInstance->SetTextPointer(static_cast<Font*>(pResource));
...
//					SPDLOG_DEBUG("\t");
				...
				SPDLOG_DEBUG("%-10s: %2d\t", rProfileStackData.strName.c_str(), rProfileStackData.iEndTime - rProfileStackData.iStartTime);
			}
			SPDLOG_DEBUG("\n");
		}
		...
		void ProfileOneAccumulationDataByConsole(const char * c_szName)
		{
			...
			TProfileAccumulationData & rData = itor->second;
			SPDLOG_DEBUG("%-10s : [CollapsedTime : %3d] / [CallingCount : %3d]\n", rData.strName.c_str(),
