#ifndef METIN2_CLIENT_ETERLIB_GRPSCREEN_HPP
#define METIN2_CLIENT_ETERLIB_GRPSCREEN_HPP
...
public:
    ...
    void RenderGradationBar2d(float sx, float sy, float ex, float ey, DWORD dwStartColor, DWORD dwEndColor,
                              float ez = 0.0f);
    ...
    void RenderLine3d(float sx, float sy, float sz, float ex, float ey, float ez);
        void RenderGradationLine3d(float sx, float sy, float ex, float ey, DWORD dwStartColor, DWORD dwEndColor,
                              float ez = 0.0f);
    ...
    void RenderGradationBar3d(float sx, float sy, float sz, float ex, float ey, float ez, DWORD dwStartColor,
                              DWORD dwEndColor);
    ...
    void RenderTextureBox(float sx, float sy, float ex, float ey, float z = 0.0f, float su = 0.0f, float sv = 0.0f,
                          float eu = 1.0f, float ev = 1.0f);
    void RenderBillboard(Vector3 *Position, DirectX::SimpleMath::Color &Color);

    void DrawMinorGrid(float xMin, float yMin, float xMax, float yMax, float xminorStep, float yminorStep,
                       float zPos = 0);
    void DrawGrid(float xMin, float yMin, float xMax, float yMax, float xmajorStep, float ymajorStep, float xminorStep,
                  float yminorStep, float zPos = 0);

    /*void RenderD3DXMesh(LPD3DXMESH lpMesh, const Matrix *c_pmatWorld, float fx, float fy, float fz, float fRadius,
                        D3DFILLMODE d3dFillMode);*/
    void RenderSphere(const Matrix *c_pmatWorld, float fx, float fy, float fz, float fRadius,
                      D3DFILLMODE d3dFillMode = D3DFILL_SOLID);
    void RenderCylinder(const Matrix *c_pmatWorld, float fx, float fy, float fz, float fRadius, float fLength,
                        D3DFILLMODE d3dFillMode = D3DFILL_SOLID);

    void SetColorOperation();
    void SetDiffuseOperation();
    void SetBlendOperation();
    void SetOneColorOperation(DirectX::SimpleMath::Color &rColor);
    void SetAddColorOperation(DirectX::SimpleMath::Color &rColor);
    ...
    static BoundingFrustum &GetFrustum()
    {
        return ms_frustum;
    }
...
protected:
    ...
    static BoundingFrustum ms_frustum;
};
