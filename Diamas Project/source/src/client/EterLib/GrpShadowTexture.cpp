...
bool CGraphicShadowTexture::Create(int width, int height)
{
    ...
    if (FAILED(
        ms_lpd3dDevice->CreateTexture(m_width, m_height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_R32F, D3DPOOL_DEFAULT, &
            m_lpd3dShadowTexture, NULL)))
        return false;
    ...
    if (FAILED(
        ms_lpd3dDevice->CreateDepthStencilSurface(m_width, m_height, D3DFMT_R32F, D3DMULTISAMPLE_NONE, 0, false, &
            m_lpd3dDepthSurface, NULL)))
        return false;
...
{
    m_d3dLightVPMatrix = ms_matView * ms_matProj;
