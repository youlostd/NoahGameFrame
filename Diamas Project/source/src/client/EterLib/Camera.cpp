...
void CCamera::SetViewMatrix()
{
    ...
    CalculateRoll();
    m_fDistance = m_v3View.Length();
    assert(m_fDistance >= 0);

    m_v3View.Normalize();
    m_v3Cross = m_v3Up.Cross(m_v3View);
    m_v3Cross.Normalize();
    m_v3Up = m_v3View.Cross(m_v3Cross);
    m_v3Up.Normalize(m_v3Up);

    m_fPitch = m_v3Up.Dot(Vector3::Backward); // / D3DXVec2Length(&v2ViewYZ);
    if (m_fPitch >= 1)
        m_fPitch = 1;
    else if (m_fPitch <= -1)
        m_fPitch = -1;
    ...
    m_matView = Matrix::CreateLookAt(m_v3Eye, m_v3Target, m_v3Up);

    auto fDeterminantD3DMatView = XMMatrixDeterminant(m_matView);
    m_matInverseView = XMMatrixInverse(&fDeterminantD3DMatView, m_matView);
    ...
    m_kTargetToCameraBottomRay.SetDirection(v3CenterRay - m_fTerrainCollisionRadius * m_v3Up,
                                            ((v3CenterRay - m_fTerrainCollisionRadius * m_v3Up).Length()));
    ...
    m_kLeftObjectCollisionRay.SetDirection(v3CenterRay + m_fObjectCollisionRadius * m_v3Cross,
                                           ((v3CenterRay + m_fObjectCollisionRadius * m_v3Cross).Length()));
    ...
    m_kTopObjectCollisionRay.SetDirection(v3CenterRay + m_fObjectCollisionRadius * m_v3Up,
                                          ((v3CenterRay + m_fObjectCollisionRadius * m_v3Up).Length()));
...
//{
//	D3DXMATRIX matRot;
//	D3DXMatrixRotationAxis(&matRot, &m_v3Cross, -XMConvertToRadians(fDegree));
...
void CCamera::RotateEyeAroundTarget(float fPitchDegree, float fRollDegree)
{
    ...
    matRotPitch = Matrix::CreateFromAxisAngle(m_v3Cross, XMConvertToRadians(fPitchDegree));

    matRotRoll = Matrix::CreateRotationZ(-XMConvertToRadians(fRollDegree));
    matRot = matRotPitch * matRotRoll;

    Vector3 v3Temp = m_v3Eye - m_v3Target;
    m_v3Eye = Vector3::Transform(v3Temp, matRot);
    m_v3Eye += m_v3Target;
...
void CCamera::RotateEyeAroundPoint(const Vector3 &v3Point, float fPitchDegree, float fRollDegree)
{
    ...
    m_v3Up = Vector3::Transform((v3Temp + m_v3Up), matRot);
    m_v3Up -= (m_v3Eye - v3Point);

    v3Temp = m_v3Target - v3Point;
    m_v3Target = Vector3::Transform(v3Temp, matRot);
    m_v3Target += v3Point;
...
void CCamera::CalculateRoll()
{
    ...
    v2ViewXY.Normalize();
    float fDot = v2ViewXY.Dot(Vector2::UnitY);
    if (fDot >= 1)
        fDot = 1;
    else if (fDot <= -1)
        fDot = -1;
    fDot = acosf(fDot);
    fDot *= (180.0f / XM_PI);
    float fCross = v2ViewXY.Cross(Vector2::UnitY).x;
    if (0 > fCross)
    {
        fDot = -fDot;
    }
      ...
      m_pPreviousCamera(NULL)
{
    ...
    AddCamera(ILLUSTRATION_CAMERA);
    AddCamera(SHOPDECO_CAMERA);
    AddCamera(DEFAULT_WATER_REFLECTION_CAM);
...
void CCameraManager::SetCurrentCamera(uint32_t ucCameraNum)
{
    if (m_CameraMap.find(ucCameraNum) == m_CameraMap.end())
        AddCamera(ucCameraNum);

    if (m_pCurrentCamera != m_CameraMap[ucCameraNum].get())
        m_pPreviousCamera = m_pCurrentCamera;

    m_pCurrentCamera = m_CameraMap[ucCameraNum].get();
}
...
bool CCameraManager::isCurrentCamera(uint32_t ucCameraNum)
{
    if (m_CameraMap[ucCameraNum].get() == m_pCurrentCamera)
        return true;
    return false;
}

// 잡스러운 함수들...
bool CCameraManager::AddCamera(uint32_t ucCameraNum)
{
    if (m_CameraMap.end() != m_CameraMap.find(ucCameraNum))
        return false;
    m_CameraMap.emplace(ucCameraNum, std::make_unique<CCamera>());
    return true;
}

bool CCameraManager::RemoveCamera(uint32_t ucCameraNum)
{
...
uint32_t CCameraManager::GetCurrentCameraNum()
{
    if (!m_pCurrentCamera)
        return NO_CURRENT_CAMERA;
    for (auto &itor : m_CameraMap)
        if (m_pCurrentCamera == itor.second.get())
            return itor.first;
    return NO_CURRENT_CAMERA;
}
