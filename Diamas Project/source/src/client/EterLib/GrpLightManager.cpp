...
void CLightManager::FlushLight()
{
    ...
    for (; itor != m_LightMap.end(); ++itor)
    {
        ...
        Vector3 v3LightPos((Vector3)pLight->GetPosition());
        Vector3 v3Distance(v3LightPos - m_v3CenterPosition);
        pLight->SetDistance(v3Distance.Length());
...
{
    return *(Vector3*)(&m_d3dLight.Position);
}
...
void CLight::BlendDiffuseColor(const DirectX::SimpleMath::Color &c_rColor, float fBlendTime, float fDelayTime)
{
    DirectX::SimpleMath::Color Color = {m_d3dLight.Diffuse.r, m_d3dLight.Diffuse.g, m_d3dLight.Diffuse.b, m_d3dLight.Diffuse.a};
    m_DiffuseColorTransitor.SetTransition(Color, c_rColor, ms_fCurTime + fDelayTime, fBlendTime);
}

void CLight::BlendAmbientColor(const DirectX::SimpleMath::Color &c_rColor, float fBlendTime, float fDelayTime)
{
   // DirectX::SimpleMath::Color Color(m_d3dLight.Ambient);
