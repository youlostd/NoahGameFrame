...
void CPrivManager::Update()
{
	...
	while (!m_pqPrivGuild.empty() && m_pqPrivGuild.top().first <= now) {
		auto p = m_pqPrivGuild.top().second;
		m_pqPrivGuild.pop();
	...
	while (!m_pqPrivEmpire.empty() && m_pqPrivEmpire.top().first <= now) {
		auto p = (m_pqPrivEmpire.top().second);
		m_pqPrivEmpire.pop();
	...
	while (!m_pqPrivChar.empty() && m_pqPrivChar.top().first <= now) {
		auto p = (m_pqPrivChar.top().second);
		m_pqPrivChar.pop();
...
void CPrivManager::AddCharPriv(uint32_t pid, uint8_t type, int value)
{
	if (MAX_PRIV_NUM <= type) {
		spdlog::error("PRIV_MANAGER: AddCharPriv: wrong char priv type(%u) "
		              "recved",
		              type);
		return;
	}
	...
	auto p = std::make_shared<TPrivCharData>(type, value, pid);
	...
	m_pqPrivChar.push(std::make_pair(now + std::chrono::seconds(iDuration), p));
...
//
void CPrivManager::AddGuildPriv(uint32_t guild_id, uint8_t type, int value,
                                uint32_t duration_sec)
{
	if (MAX_PRIV_NUM <= type) {
		spdlog::error("PRIV_MANAGER: AddGuildPriv: wrong guild priv type(%u) "
		              "recved",
		              type);
		return;
	}
	...
	auto now = std::chrono::system_clock::now();;
	auto end = now + std::chrono::seconds(duration_sec);
	auto p = std::make_shared<TPrivGuildData>(
	    type, value, guild_id,
	...
	SPDLOG_INFO("Guild Priv guild(%d) type(%d) value(%d) duration_sec(%d)",
	            guild_id, type, value, duration_sec);
}

void CPrivManager::AddEmpirePriv(uint8_t empire, uint8_t type, int value,
                                 uint32_t duration_sec)
{
	if (MAX_PRIV_NUM <= type) {
		spdlog::error("PRIV_MANAGER: AddEmpirePriv: wrong empire priv type(%u) "
		              "recved",
		              type);
		return;
	}
	...
	auto p = std::make_shared<TPrivEmpireData>(
	    type, value, empire,
	    std::chrono::duration_cast<std::chrono::seconds>(end.time_since_epoch())
	        .count());
	...
	SendChangeEmpirePriv(
	    empire, type, value,
	    std::chrono::duration_cast<std::chrono::seconds>(end.time_since_epoch())
	        .count());
	// END_OF_ADD_EMPIRE_PRIV_TIME

	SPDLOG_INFO("Empire Priv empire(%d) type(%d) value(%d) duration_sec(%d)",
...
struct FSendChangeGuildPriv {
	FSendChangeGuildPriv(uint32_t guild_id, uint8_t type, int value,
	...
	{
		peer->EncodeHeader(HEADER_DG_CHANGE_GUILD_PRIV, 0,
		                   sizeof(TPacketDGChangeGuildPriv));
...
struct FSendChangeEmpirePriv {
	FSendChangeEmpirePriv(uint8_t empire, uint8_t type, int value,
	                      uint32_t end_time_sec)
	{
		p.empire = empire;
		p.type = type;
	...
	void operator()(CPeer* peer)
	{
		peer->EncodeHeader(HEADER_DG_CHANGE_EMPIRE_PRIV, 0,
		                   sizeof(TPacketDGChangeEmpirePriv));
...
struct FSendChangeCharPriv {
	...
	void operator()(CPeer* peer)
	{
		peer->EncodeHeader(HEADER_DG_CHANGE_CHARACTER_PRIV, 0,
		                   sizeof(TPacketDGChangeCharacterPriv));
...
// ADD_GUILD_PRIV_TIME
void CPrivManager::SendChangeGuildPriv(uint32_t guild_id, uint8_t type,
                                       int value, uint32_t end_time_sec)
{
	CClientManager::instance().for_each_peer(
	    FSendChangeGuildPriv(guild_id, type, value, end_time_sec));
}
// END_OF_ADD_GUILD_PRIV_TIME

// ADD_EMPIRE_PRIV_TIME
void CPrivManager::SendChangeEmpirePriv(uint8_t empire, uint8_t type, int value,
                                        uint32_t end_time_sec)
{
	CClientManager::instance().for_each_peer(
	    FSendChangeEmpirePriv(empire, type, value, end_time_sec));
}
// END_OF_ADD_EMPIRE_PRIV_TIME
...
{
	CClientManager::instance().for_each_peer(
	    FSendChangeCharPriv(pid, type, value));
}
...
void CPrivManager::SendPrivOnSetup(CPeer* peer)
{
	for (int i = 1; i < MAX_PRIV_NUM; ++i) {
		for (int e = 0; e < EMPIRE_MAX_NUM; ++e) {
			// ADD_EMPIRE_PRIV_TIME
			auto pPrivEmpireData = m_aaPrivEmpire[i][e];
			if (pPrivEmpireData) {
				FSendChangeEmpirePriv(e, i, pPrivEmpireData->value,
				                      pPrivEmpireData->end_time_sec)(peer);
			}
			// END_OF_ADD_EMPIRE_PRIV_TIME
		}

		for (auto it = m_aPrivGuild[i].begin(); it != m_aPrivGuild[i].end();
		     ++it) {
			// ADD_GUILD_PRIV_TIME
			FSendChangeGuildPriv(it->first, i, it->second->value,
			                     it->second->end_time_sec)(peer);
			// END_OF_ADD_GUILD_PRIV_TIME
		}
		for (auto it = m_aPrivChar[i].begin(); it != m_aPrivChar[i].end();
		     ++it) {
