...
int oxevent_open(lua_State *L)
{
    ...
    char script[256];
    std::snprintf(script, sizeof(script), "%s/oxquiz.lua",
                  GetLocaleService().GetDefaultLocale().path.c_str());
...
int oxevent_quiz(lua_State *L)
{
    if (lua_isnumber(L, 1) && lua_isnumber(L, 2))
    {
        ...
        if (ret == false) { lua_pushnumber(L, 0); }
        else { lua_pushnumber(L, 1); }
    }
...
int oxevent_end_event(lua_State *L)
{
    ...
    event_create(end_oxevent, info, THECORE_SECS_TO_PASSES(5));
