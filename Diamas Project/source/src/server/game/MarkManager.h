#ifndef METIN2_SERVER_GAME_MARKMANAGER_HPP
#define METIN2_SERVER_GAME_MARKMANAGER_HPP
...
public:
    ...
    std::vector<MarkIndex> CopyMarkIdx() const;
...
private:
    ...
    std::unordered_map<uint32_t, std::unique_ptr<CGuildMarkImage>> m_mapIdx_Image; // index = image index
    std::unordered_map<uint32_t, uint32_t> m_mapGID_MarkID;                        // index = guild id
