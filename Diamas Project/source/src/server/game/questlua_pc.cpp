#include "config.h"
#include "questmanager.h"
#include "sectree_manager.h"
#include "char.h"
#include "char_manager.h"
#include <game/AffectConstants.hpp>
#include "item.h"
#include "item_manager.h"
#include "guild_manager.h"
#include "war_map.h"
#include "start_position.h"
#include "marriage.h"
#include "mining.h"

#include "polymorph.h"
#include "DbCacheSocket.hpp"
#include "messenger_manager.h"
#include "log.h"
#include "utils.h"
#include "mob_manager.h"
#include "pvp.h"
#include "GBattle.h"
#include "desc.h"
#include <cctype>
#include "CharUtil.hpp"
#include "guild.h"
#include "MeleyLair.h"
#ifdef ENABLE_BATTLE_PASS
#include "battle_pass.h"
#endif

namespace quest
{
//
// "pc" Lua functions
//
int pc_has_master_skill(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    bool bHasMasterSkill = false;
    for (int i = 0; i < SKILL_MAX_NUM; i++)
        if (ch->GetSkillMasterType(i) >= SKILL_MASTER && ch->GetSkillLevel(i) >= 21)
        {
            bHasMasterSkill = true;
            break;
        }

    lua_pushboolean(L, bHasMasterSkill);
    return 1;
}

int pc_remove_skill_book_no_delay(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->RemoveAffect(AFFECT_SKILL_NO_BOOK_DELAY);
    return 0;
}

int pc_is_skill_book_no_delay(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    lua_pushboolean(L, ch->FindAffect(AFFECT_SKILL_NO_BOOK_DELAY) ? true : false);
    return 1;
}

int pc_learn_grand_master_skill(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("wrong skill index");
        return 0;
    }

    lua_pushboolean(L, ch->LearnGrandMasterSkill((long)lua_tonumber(L, 1)));
    return 1;
}

int pc_set_warp_location(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("wrong map index");
        return 0;
    }

    if (!lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        SPDLOG_ERROR("wrong coodinate");
        return 0;
    }

    ch->SetWarpLocation((long)lua_tonumber(L, 1), (long)lua_tonumber(L, 2), (long)lua_tonumber(L, 3));
    return 0;
}

int pc_set_warp_location_local(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("wrong map index");
        return 0;
    }

    if (!lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        SPDLOG_ERROR("wrong coodinate");
        return 0;
    }

    long lMapIndex = (long)lua_tonumber(L, 1);
    const TMapRegion *region = SECTREE_MANAGER::instance().GetMapRegion(lMapIndex);

    if (!region)
    {
        SPDLOG_ERROR("invalid map index {}", lMapIndex);
        return 0;
    }

    int x = (int)lua_tonumber(L, 2);
    int y = (int)lua_tonumber(L, 3);

    if (x > region->width)
    {
        SPDLOG_ERROR("x coordinate overflow max: {} input: {}", region->width, x);
        return 0;
    }

    if (y > region->height)
    {
        SPDLOG_ERROR("y coordinate overflow max: {} input: {}", region->height, y);
        return 0;
    }

    ch->SetWarpLocation(lMapIndex, x, y);
    return 0;
}

int pc_get_start_location(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    lua_pushnumber(L, g_start_position[ch->GetEmpire()][2]);
    lua_pushnumber(L, g_start_position[ch->GetEmpire()][0] / 100);
    lua_pushnumber(L, g_start_position[ch->GetEmpire()][1] / 100);
    return 3;
}

int pc_warp(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        lua_pushboolean(L, false);
        return 1;
    }

    // PREVENT_HACK
    if (ch->IsHack())
    {
        lua_pushboolean(L, false);
        return 1;
    }
    // END_PREVENT_HACK

    if (gConfig.testServer)
        ch->ChatPacket(CHAT_TYPE_INFO, "pc_warp %d (%d, %d)", (int)lua_tonumber(L, 1), (int)lua_tonumber(L, 2),
                       (int)lua_tonumber(L, 3));

    ch->WarpSet((int)lua_tonumber(L, 1), (int)lua_tonumber(L, 2), (int)lua_tonumber(L, 3));

    lua_pushboolean(L, true);

    return 1;
}
int pc_warp_force(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        lua_pushboolean(L, false);
        return 1;
    }


    if (gConfig.testServer)
        ch->ChatPacket(CHAT_TYPE_INFO, "pc_warp_force %d (%d, %d)", (int)lua_tonumber(L, 1), (int)lua_tonumber(L, 2),
                       (int)lua_tonumber(L, 3));

    ch->WarpSet((int)lua_tonumber(L, 1), (int)lua_tonumber(L, 2), (int)lua_tonumber(L, 3));

    lua_pushboolean(L, true);

    return 1;
}

int pc_warp_local(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("no map index argument");
        return 0;
    }

    if (!lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        SPDLOG_ERROR("no coodinate argument");
        return 0;
    }

    long lMapIndex = (long)lua_tonumber(L, 1);
    const TMapRegion *region = SECTREE_MANAGER::instance().GetMapRegion(lMapIndex);

    if (!region)
    {
        SPDLOG_ERROR("invalid map index %d", lMapIndex);
        return 0;
    }

    int x = (int)lua_tonumber(L, 2);
    int y = (int)lua_tonumber(L, 3);

    if (x > region->width)
    {
        SPDLOG_ERROR("x coordinate overflow max: %d input: %d", region->width, x);
        return 0;
    }

    if (y > region->height)
    {
        SPDLOG_ERROR("y coordinate overflow max: %d input: %d", region->height, y);
        return 0;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch)
    {
        ch->WarpSet(ch->GetMapIndex(), x, y);
    }

    return 0;
}

int pc_warp_exit(lua_State *L)
{
    CQuestManager::instance().GetCurrentCharacterPtr()->ExitToSavedLocation();
    return 0;
}

int pc_in_dungeon(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    auto *d = ch->GetDungeon();
    if (!d)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    if (lua_gettop(L) >= 1)
    {
        auto mapIndex = d->GetOriginalMapIndex();
        auto dungeonMapIndex = lua_tonumber(L, 1);

        lua_pushboolean(L, mapIndex == dungeonMapIndex);
        return 1;
    }

    lua_pushboolean(L, 1);
    return 1;
}

int pc_hasguild(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, ch->GetGuild() ? 1 : 0);
    return 1;
}

int pc_getguild(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetGuild() ? ch->GetGuild()->GetID() : 0);
    return 1;
}

int pc_isguildmaster(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CGuild *g = ch->GetGuild();

    if (g)
        lua_pushboolean(L, (ch->GetPlayerID() == g->GetMasterPID()));
    else
        lua_pushboolean(L, 0);

    return 1;
}

int pc_destroy_guild(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CGuild *g = ch->GetGuild();

    if (g && !g->UnderAnyWar())
        g->RequestDisband(ch->GetPlayerID());

    return 0;
}

int pc_remove_from_guild(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CGuild *g = ch->GetGuild();

    if (g)
        g->RequestRemoveMember(ch->GetPlayerID());

    return 0;
}

int pc_give_gold(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("QUEST : wrong argument");
        return 0;
    }

    Gold iAmount = (Gold)lua_tonumber(L, 1);

    if (iAmount <= 0)
    {
        SPDLOG_ERROR("QUEST : gold amount less then zero");
        return 0;
    }

    DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), iAmount);
    ch->ChangeGold(iAmount);
    return 0;
}

int pc_warp_to_guild_war_observer_position(lua_State *L)
{
    luaL_checknumber(L, 1);
    luaL_checknumber(L, 2);

    uint32_t gid1 = (uint32_t)lua_tonumber(L, 1);
    uint32_t gid2 = (uint32_t)lua_tonumber(L, 2);

    CGuild *g1 = CGuildManager::instance().FindGuild(gid1);
    CGuild *g2 = CGuildManager::instance().FindGuild(gid2);

    if (!g1 || !g2)
    {
        luaL_error(L, "no such guild with id %d %d", gid1, gid2);
    }

    PIXEL_POSITION pos;

    uint32_t dwMapIndex = g1->GetGuildWarMapIndex(gid2);

    if (!CWarMapManager::instance().GetStartPosition(dwMapIndex, 2, pos))
    {
        luaL_error(L, "not under warp guild war between guild %d %d", gid1, gid2);
        return 0;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    // PREVENT_HACK
    if (ch->IsHack())
        return 0;
    // END_PREVENT_HACK

    ch->SetQuestFlag("war.is_war_member", 0);
    ch->SaveExitLocation();
    ch->WarpSet(dwMapIndex, pos.x, pos.y);
    return 0;
}

int pc_give_item_from_special_item_group(lua_State *L)
{
    luaL_checknumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    uint32_t dwGroupVnum = (uint32_t)lua_tonumber(L, 1);

    std::vector<uint32_t> dwVnums;
    std::vector<uint32_t> dwCounts;
    std::vector<CItem *> item_gets(0); // c++11 compatibility change

    int count = 0;

    ch->GiveItemFromSpecialItemGroup(dwGroupVnum, dwVnums, dwCounts, item_gets, count);

    for (int i = 0; i < count; i++)
    {
        if (!item_gets[i])
        {
            if (dwVnums[i] == 2)
            {
                SendI18nChatPacket(ch, CHAT_TYPE_INFO, "나무에서 부터 신비한 빛이 나옵니다.");
                SendI18nChatPacket(ch, CHAT_TYPE_INFO, "%d의 경험치를 획득했습니다.", dwCounts[i]);
            }
        }
    }
    return 0;
}

int pc_enough_inventory(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!lua_isnumber(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    uint32_t item_vnum = (uint32_t)lua_tonumber(L, 1);
    const TItemTable *pTable = ITEM_MANAGER::instance().GetTable(item_vnum);
    if (!pTable)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    bool bEnoughInventoryForItem = ch->GetEmptyInventory(pTable->bSize, pTable->bType) != -1;
    lua_pushboolean(L, bEnoughInventoryForItem);
    return 1;
}

int pc_give_item(lua_State *L)
{
    PC *pPC = CQuestManager::instance().GetCurrentPC();
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isstring(L, 1) || !(lua_isstring(L, 2) || lua_isnumber(L, 2)))
    {
        SPDLOG_ERROR("QUEST : wrong argument");
        return 0;
    }

    uint32_t dwVnum;

    if (lua_isnumber(L, 2)) // 번호인경우 번호로 준다.
        dwVnum = (int)lua_tonumber(L, 2);
    else if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 2), dwVnum))
    {
        SPDLOG_ERROR("QUEST Make item call error : wrong item name : %s", lua_tostring(L, 1));
        return 0;
    }

    int icount = 1;

    if (lua_isnumber(L, 3) && lua_tonumber(L, 3) > 0)
    {
        icount = (int)rint(lua_tonumber(L, 3));

        if (icount <= 0)
        {
            SPDLOG_ERROR("QUEST Make item call error : wrong item count : %g", lua_tonumber(L, 2));
            return 0;
        }
    }

    pPC->GiveItem(lua_tostring(L, 1), dwVnum, icount);

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), dwVnum,
                                          icount);
    return 0;
}

int pc_give_or_drop_item(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isstring(L, 1) && !lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("QUEST Make item call error : wrong argument");
        lua_pushnumber(L, 0);
        return 1;
    }

    uint32_t dwVnum;

    if (lua_isnumber(L, 1)) // 번호인경우 번호로 준다.
    {
        dwVnum = (uint32_t)lua_tonumber(L, 1);
    }
    else if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 1), dwVnum))
    {
        SPDLOG_ERROR("QUEST Make item call error : wrong item name : %s", lua_tostring(L, 1));
        lua_pushnumber(L, 0);

        return 1;
    }

    int icount = 1;
    if (lua_isnumber(L, 2) && lua_tonumber(L, 2) > 0)
    {
        icount = (int)rint(lua_tonumber(L, 2));
        if (icount <= 0)
        {
            SPDLOG_ERROR("QUEST Make item call error : wrong item count : %g", lua_tonumber(L, 2));
            lua_pushnumber(L, 0);
            return 1;
        }
    }

    SPDLOG_TRACE("QUEST [REWARD] item {} to {}", lua_tostring(L, 1), ch->GetName());

    PC *pPC = CQuestManager::instance().GetCurrentPC();

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), dwVnum,
                                          icount);

    CItem *item = ch->AutoGiveItem(dwVnum, icount);

    if ((dwVnum >= 80003 && dwVnum <= 80007) || (dwVnum >= 80018 && dwVnum <= 80020))
    {
        LogManager::instance().GoldBarLog(ch->GetPlayerID(), item->GetID(), QUEST, "quest: give_item2"); //-V595
    }

    if (nullptr != item)
        lua_pushnumber(L, item->GetID());
    else
        lua_pushnumber(L, 0);
    return 1;
}

int pc_give_or_drop_item_and_select(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isstring(L, 1) && !lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("QUEST Make item call error : wrong argument");
        return 0;
    }

    uint32_t dwVnum;

    if (lua_isnumber(L, 1)) // 번호인경우 번호로 준다.
    {
        dwVnum = (uint32_t)lua_tonumber(L, 1);
    }
    else if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 1), dwVnum))
    {
        SPDLOG_ERROR("QUEST Make item call error : wrong item name : %s", lua_tostring(L, 1));
        return 0;
    }

    int icount = 1;
    if (lua_isnumber(L, 2) && lua_tonumber(L, 2) > 0)
    {
        icount = (int)rint(lua_tonumber(L, 2));
        if (icount <= 0)
        {
            SPDLOG_ERROR("QUEST Make item call error : wrong item count : %g", lua_tonumber(L, 2));
            return 0;
        }
    }

    SPDLOG_TRACE("QUEST [REWARD] item {} to {}", lua_tostring(L, 1), ch->GetName());

    PC *pPC = CQuestManager::instance().GetCurrentPC();

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), dwVnum,
                                          icount);

    CItem *item = ch->AutoGiveItem(dwVnum, icount);

    if (nullptr != item)
    {
        CQuestManager::instance().UpdateQuestItem(ch, item);
    }

    if ((dwVnum >= 80003 && dwVnum <= 80007) || (dwVnum >= 80018 && dwVnum <= 80020))
    {
        LogManager::instance().GoldBarLog(ch->GetPlayerID(), item->GetID(), QUEST, "quest: give_item2");
    }

    return 0;
}

int pc_get_current_map_index(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetMapIndex());
    return 1;
}

int pc_get_x(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetX() / 100);
    return 1;
}

int pc_get_y(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetY() / 100);
    return 1;
}

int pc_get_local_x(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetX() / 100);
    return 1;
}

int pc_get_local_y(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    lua_pushnumber(L, ch->GetY() / 100);
    return 1;
}

int pc_count_item(lua_State *L)
{
    if (lua_isnumber(L, -1))
        lua_pushnumber(
            L, CQuestManager::instance().GetCurrentCharacterPtr()->CountSpecifyItem((uint32_t)lua_tonumber(L, -1)));
    else if (lua_isstring(L, -1))
    {
        uint32_t item_vnum;

        if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 1), item_vnum))
        {
            SPDLOG_ERROR("QUEST count_item call error : wrong item name : %s", lua_tostring(L, 1));
            lua_pushnumber(L, 0);
        }
        else
        {
            lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->CountSpecifyItem(item_vnum));
        }
    }
    else
        lua_pushnumber(L, 0);

    return 1;
}

int pc_find_first_item(lua_State *L)
{
    if (!lua_isnumber(L, -1))
    {
        lua_pushnumber(L, -1);
        return 1;
    }

    CItem *item = CQuestManager::instance().GetCurrentCharacterPtr()->FindSpecifyItem((uint32_t)lua_tonumber(L, -1));
    if (!item)
    {
        lua_pushnumber(L, -1);
        return 1;
    }

    lua_pushnumber(L, item->GetCell());
    return 1;
}

int pc_remove_item(lua_State *L)
{
    if (lua_gettop(L) == 1)
    {
        uint32_t item_vnum;

        if (lua_isnumber(L, 1))
        {
            item_vnum = (uint32_t)lua_tonumber(L, 1);
        }
        else if (lua_isstring(L, 1))
        {
            if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 1), item_vnum))
            {
                SPDLOG_ERROR("QUEST remove_item call error : wrong item name : %s", lua_tostring(L, 1));
                return 0;
            }
        }
        else
        {
            SPDLOG_ERROR("QUEST remove_item wrong argument");
            return 0;
        }

        auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
        if (ch)
            ch->RemoveSpecifyItem(item_vnum);
    }
    else if (lua_gettop(L) == 2)
    {
        uint32_t item_vnum;

        if (lua_isnumber(L, 1))
        {
            item_vnum = (uint32_t)lua_tonumber(L, 1);
        }
        else if (lua_isstring(L, 1))
        {
            if (!ITEM_MANAGER::instance().GetVnum(lua_tostring(L, 1), item_vnum))
            {
                SPDLOG_ERROR("QUEST remove_item call error : wrong item name : {}", lua_tostring(L, 1));
                return 0;
            }
        }
        else
        {
            SPDLOG_ERROR("QUEST remove_item wrong argument");
            return 0;
        }
        auto ch = CQuestManager::instance().GetCurrentCharacterPtr();

        uint32_t item_count = (uint32_t)lua_tonumber(L, 2);
        if (ch)
        {
            SPDLOG_TRACE("QUEST remove items(vnum %d) count %d of %s[%d]", item_vnum, item_count,
                        CQuestManager::instance().GetCurrentCharacterPtr()->GetName(),
                        CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerID());

            ch->RemoveSpecifyItem(item_vnum, item_count);
        }
    }
    return 0;
}

int pc_get_leadership(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetLeadershipSkillLevel());
    return 1;
}

int pc_reset_point(lua_State *L)
{
    CQuestManager::instance().GetCurrentCharacterPtr()->ResetPoint(
        CQuestManager::instance().GetCurrentCharacterPtr()->GetLevel());
    return 0;
}

int pc_get_playtime(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetRealPoint(POINT_PLAYTIME));
    return 1;
}

int pc_get_vid(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetVID());
    return 1;
}

int pc_get_pid(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPlayerID());
    return 1;
}

int pc_get_name(lua_State *L)
{
    lua_pushstring(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetName().c_str());
    return 1;
}

int pc_get_next_exp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetNextExp());
    return 1;
}

int pc_get_exp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetExp());
    return 1;
}

int pc_get_race(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetRaceNum());
    return 1;
}

int pc_change_sex(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->ChangeSex());
    return 1;
}

int pc_get_job(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetJob());
    return 1;
}

int pc_get_max_sp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetMaxSP());
    return 1;
}

int pc_get_sp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetSP());
    return 1;
}

int pc_change_sp(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("invalid argument");
        lua_pushboolean(L, 0);
        return 1;
    }

    long val = (long)lua_tonumber(L, 1);

    if (val == 0)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (val > 0) // 증가시키는 것이므로 무조건 성공 리턴
        ch->PointChange(POINT_SP, val);
    else if (val < 0)
    {
        if (ch->GetSP() < -val)
        {
            lua_pushboolean(L, 0);
            return 1;
        }

        ch->PointChange(POINT_SP, val);
    }

    lua_pushboolean(L, 1);
    return 1;
}

int pc_get_max_hp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetMaxHP());
    return 1;
}

int pc_get_hp(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetHP());
    return 1;
}

int pc_get_level(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetLevel());
    return 1;
}

int pc_get_hwid(lua_State *L)
{
    lua_pushstring(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetDesc()->GetHWIDHash().c_str());
    return 1;
}

int pc_set_level(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("invalid argument");
        return 0;
    }
    else
    {
        int newLevel = (int)lua_tonumber(L, 1);
        CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

        SPDLOG_TRACE("QUEST [LEVEL] %s jumpint to level %d", ch->GetName(), (int)rint(lua_tonumber(L, 1)));

        PC *pPC = CQuestManager::instance().GetCurrentPC();
        LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(),
                                              newLevel, 0);

        //포인트 : 스킬, 서브스킬, 스탯
        ch->PointChange(POINT_SKILL, newLevel - ch->GetLevel());
        ch->PointChange(POINT_SUB_SKILL, newLevel < 10 ? 0 : newLevel - std::max<int>(ch->GetLevel(), 9));
        ch->PointChange(POINT_STAT, ((std::clamp(newLevel, 1, gConfig.maxLevelStats) - ch->GetLevel()) * 3) +
                                        ch->GetPoint(POINT_LEVEL_STEP));
        //레벨
        ch->PointChange(POINT_LEVEL, newLevel - ch->GetLevel());
        // HP, SP
        ch->SetRandomHP((newLevel - 1) * Random::get(JobInitialPoints[ch->GetJob()].hp_per_lv_begin,
                                                     JobInitialPoints[ch->GetJob()].hp_per_lv_end));
        ch->SetRandomSP((newLevel - 1) * Random::get(JobInitialPoints[ch->GetJob()].sp_per_lv_begin,
                                                     JobInitialPoints[ch->GetJob()].sp_per_lv_end));

        // 회복
        ch->PointChange(POINT_HP, ch->GetMaxHP() - ch->GetHP());
        ch->PointChange(POINT_SP, ch->GetMaxSP() - ch->GetSP());

        ch->ComputePoints();
        ch->ComputeMountPoints();
        ch->PointsPacket();
        ch->SkillLevelPacket();

        return 0;
    }
}

int pc_get_weapon(lua_State *L)
{
    CItem *item = CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_WEAPON);

    if (!item)
        lua_pushnumber(L, 0);
    else
        lua_pushnumber(L, item->GetVnum());

    return 1;
}

int pc_get_armor(lua_State *L)
{
    CItem *item = CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_BODY);

    if (!item)
        lua_pushnumber(L, 0);
    else
        lua_pushnumber(L, item->GetVnum());

    return 1;
}

int pc_get_wear(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("QUEST wrong set flag");
        return 0;
    }

    uint16_t bCell = (uint16_t)lua_tonumber(L, 1);

    CItem *item = CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(bCell);

    if (!item)
        lua_pushnil(L);
    else
        lua_pushnumber(L, item->GetVnum());

    return 1;
}

int pc_get_money(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetGold());
    return 1;
}

// 20050725.myevan.은둔의 망토 사용중 혼석 수련시 선악치가 두배 소모되는 버그가 발생해
// 실제 선악치를 이용해 계산을 하게 한다.
int pc_get_real_alignment(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetRealAlignment() / 10);
    return 1;
}

int pc_get_alignment(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetAlignment() / 10);
    return 1;
}

int pc_change_alignment(lua_State *L)
{
    int alignment = (int)(lua_tonumber(L, 1) * 10);
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    ch->UpdateAlignment(alignment);
    return 0;
}

bool MultiplicateSafe(uint32_t a, uint32_t b)
{
    size_t a_bits = (size_t)log(a);
    size_t b_bits = (size_t)log(b);

    return (a_bits + b_bits <= 32);
}

int pc_change_money(lua_State *L)
{
    Gold gold = (Gold)lua_tonumber(L, -1);
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (gold + ch->GetGold() < 0)
        SPDLOG_ERROR("QUEST wrong ChangeGold %lld (now %lld)", gold, ch->GetGold());
    else
    {
        DBManager::instance().SendMoneyLog(MONEY_LOG_QUEST, ch->GetPlayerID(), gold);
        ch->ChangeGold(gold);
    }

    return 0;
}

int pc_set_another_quest_flag(lua_State *L)
{
    if (!lua_isstring(L, 1) || !lua_isstring(L, 2) || !lua_isnumber(L, 3))
    {
        SPDLOG_ERROR("QUEST wrong set flag");
        return 0;
    }
    else
    {
        const char *sz = lua_tostring(L, 1);
        const char *sz2 = lua_tostring(L, 2);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        pPC->SetFlag(string(sz) + "." + sz2, int(rint(lua_tonumber(L, 3))));
        return 0;
    }
}

int pc_get_another_quest_flag(lua_State *L)
{
    if (!lua_isstring(L, 1) || !lua_isstring(L, 2))
    {
        SPDLOG_ERROR("QUEST wrong get flag");
        return 0;
    }
    else
    {
        const char *sz = lua_tostring(L, 1);
        const char *sz2 = lua_tostring(L, 2);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        if (!pPC)
        {
            return 0;
        }
        lua_pushnumber(L, pPC->GetFlag(string(sz) + "." + sz2));
        return 1;
    }
}

int pc_get_flag(lua_State *L)
{
    if (!lua_isstring(L, -1))
    {
        SPDLOG_ERROR("QUEST wrong get flag");
        return 0;
    }
    else
    {
        const char *sz = lua_tostring(L, -1);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        lua_pushnumber(L, pPC->GetFlag(sz));
        return 1;
    }
}

int pc_get_quest_flag(lua_State *L)
{
    if (!lua_isstring(L, -1))
    {
        SPDLOG_ERROR("QUEST wrong get flag");
        return 0;
    }
    else
    {
        const char *sz = lua_tostring(L, -1);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        lua_pushnumber(L, pPC->GetFlag(pPC->GetCurrentQuestName() + "." + sz));
        if (gConfig.testServer)
            SPDLOG_DEBUG("GetQF ( {} . {} )", pPC->GetCurrentQuestName().c_str(), sz);
    }
    return 1;
}

int pc_set_flag(lua_State *L)
{
    if (!lua_isstring(L, 1) || !lua_isnumber(L, 2))
    {
        SPDLOG_ERROR("QUEST wrong set flag");
    }
    else
    {
        const char *sz = lua_tostring(L, 1);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        pPC->SetFlag(sz, int(rint(lua_tonumber(L, 2))));
    }
    return 0;
}

int pc_set_quest_flag(lua_State *L)
{
    if (!lua_isstring(L, 1) || !lua_isnumber(L, 2))
    {
        SPDLOG_ERROR("QUEST wrong set flag");
    }
    else
    {
        const char *sz = lua_tostring(L, 1);
        CQuestManager &q = CQuestManager::Instance();
        PC *pPC = q.GetCurrentPC();
        pPC->SetFlag(pPC->GetCurrentQuestName() + "." + sz, int(rint(lua_tonumber(L, 2))));
    }
    return 0;
}

int pc_del_quest_flag(lua_State *L)
{
    if (!lua_isstring(L, 1))
    {
        SPDLOG_ERROR("argument error");
        return 0;
    }

    const char *sz = lua_tostring(L, 1);
    PC *pPC = CQuestManager::instance().GetCurrentPC();
    pPC->DeleteFlag(pPC->GetCurrentQuestName() + "." + sz);
    return 0;
}

int pc_give_exp2(lua_State *L)
{
    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();
    if (!lua_isnumber(L, 1))
        return 0;

    if (ch == nullptr)
    {
        return 0;
    }

    SPDLOG_TRACE("QUEST [REWARD] %s give exp2 %d", ch->GetName(), (int)rint(lua_tonumber(L, 1)));

    uint32_t exp = (uint32_t)rint(lua_tonumber(L, 1));

    PC *pPC = CQuestManager::instance().GetCurrentPC();
    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), exp,
                                          0);
    ch->PointChange(POINT_EXP, exp);
    return 0;
}

int pc_give_exp(lua_State *L)
{
    if (!lua_isstring(L, 1) || !lua_isnumber(L, 2))
        return 0;

    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();

    if (ch == nullptr)
    {
        return 0;
    }

    SPDLOG_TRACE("QUEST [REWARD] %s give exp %s %d", ch->GetName(), lua_tostring(L, 1), (int)rint(lua_tonumber(L, 2)));

    uint32_t exp = (uint32_t)rint(lua_tonumber(L, 2));

    PC *pPC = CQuestManager::instance().GetCurrentPC();

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), exp,
                                          0);

    pPC->GiveExp(lua_tostring(L, 1), exp);
    return 0;
}

int pc_give_exp_perc(lua_State *L)
{
    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();

    if (!ch || !lua_isstring(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
        return 0;

    int lev = (int)rint(lua_tonumber(L, 2));
    double proc = (lua_tonumber(L, 3));

    SPDLOG_TRACE("QUEST [REWARD] %s give exp %s lev %d percent %g%%", ch->GetName(), lua_tostring(L, 1), lev, proc);

    uint32_t exp = (uint32_t)((exp_table[std::clamp<int>(lev, 0, PLAYER_EXP_TABLE_MAX)] * proc) / 100);

    PC *pPC = CQuestManager::instance().GetCurrentPC();

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(), exp,
                                          0);

    pPC->GiveExp(lua_tostring(L, 1), exp);
    return 0;
}

int pc_get_exp_perc(lua_State *L)
{
    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
        return 0;

    int lev = (int)rint(lua_tonumber(L, 1));
    double proc = (lua_tonumber(L, 2));

    lua_pushnumber(L, (uint32_t)((exp_table[std::clamp<int>(lev, 0, PLAYER_EXP_TABLE_MAX)] * proc) / 100));
    return 1;
}

int pc_get_empire(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
    {
        lua_pushnil(L);
        return 0;
    }

    lua_pushnumber(L, ch->GetEmpire());
    return 1;
}

int pc_get_part(lua_State *L)
{
    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();
    if (!lua_isnumber(L, 1))
    {
        lua_pushnumber(L, 0);
        return 1;
    }
    if (ch == nullptr)
    {
        return 0;
    }

    int part_idx = (int)lua_tonumber(L, 1);
    lua_pushnumber(L, ch->GetPart(part_idx).vnum);
    return 1;
}

int pc_set_part(lua_State *L)
{
    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();
    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
    {
        return 0;
    }

    if (ch == nullptr)
    {
        return 0;
    }
    int index = (int)lua_tonumber(L, 1);
    Part value = {(uint32_t)lua_tonumber(L, 2), 0};

    if (lua_isnumber(L, 3))
        value.appearance = (int)lua_tonumber(L, 3);

    ch->SetPart(index, value);
    ch->UpdatePacket();
    return 0;
}
int pc_update_packet(lua_State *L)
{
    CQuestManager &q = CQuestManager::instance();
    CHARACTER *ch = q.GetCurrentCharacterPtr();

    if (ch == nullptr)
    {
        return 0;
    }

    ch->UpdatePacket();
    return 0;
}
int pc_get_skillgroup(lua_State *L)
{
    lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetSkillGroup());
    return 1;
}

int pc_change_skillgroup(lua_State *L)
{
    CQuestManager::instance().GetCurrentCharacterPtr()->ChangeSkillGroup();
    return 0;
}

int pc_set_skillgroup(lua_State *L)
{
    if (!lua_isnumber(L, 1))
        SPDLOG_ERROR("QUEST wrong skillgroup number");
    else
    {
        CQuestManager &q = CQuestManager::Instance();
        CHARACTER *ch = q.GetCurrentCharacterPtr();

        if (ch == nullptr)
        {
            return 0;
        }
        ch->SetSkillGroup((uint8_t)rint(lua_tonumber(L, 1)));
    }
    return 0;
}

int pc_is_polymorphed(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, ch->IsPolymorphed());
    return 1;
}

int pc_remove_polymorph(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
    {
        return 0;
    }

    ch->RemoveAffect(AFFECT_POLYMORPH);
    ch->SetPolymorph(0);
    return 0;
}

int pc_polymorph(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
    {
        return 0;
    }

    if (CPolymorphUtils::Instance().IsOnPolymorphMapBlacklist(ch->GetMapIndex()))
    {
        SendI18nChatPacket(ch, CHAT_TYPE_INFO, "둔갑에 실패 하였습니다");
        return 0;
    }

    uint32_t dwVnum = (uint32_t)lua_tonumber(L, 1);
    int iDuration = (int)lua_tonumber(L, 2);
    ch->AddAffect(AFFECT_POLYMORPH, POINT_POLYMORPH, dwVnum, iDuration, 0, true);
    return 0;
}

int pc_is_mount(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
    {
        lua_pushboolean(L, false);
        return 0;
    }

    lua_pushboolean(L, ch->GetMountVnum() != 0);
    return 1;
}

int pc_mount(lua_State *L)
{
    if (!lua_isnumber(L, 1))
        return 0;

    int length = 60;

    if (lua_isnumber(L, 2))
        length = (int)lua_tonumber(L, 2);

    uint32_t mount_vnum = (uint32_t)lua_tonumber(L, 1);

    if (length < 0)
        length = 60;

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!IsMountableZone(ch->GetMapIndex(), mount_vnum == ch->GetMyHorseVnum()))
    {
        SendI18nChatPacket(ch, CHAT_TYPE_INFO, "Sorry, you can't use your horse or another mount here.");
        return 0;
    }

    ch->RemoveAffect(AFFECT_MOUNT);
    ch->RemoveAffect(AFFECT_MOUNT_BONUS);

    // 말이 소환되어 따라다니는 상태라면 말부터 없앰
    if (ch->GetHorse())
        ch->HorseSummon(false);

    if (mount_vnum)
    {
        ch->AddAffect(AFFECT_MOUNT, POINT_MOUNT, mount_vnum, length, 0, true);
        switch (mount_vnum)
        {
        case 20201:
        case 20202:
        case 20203:
        case 20204:
        case 20213:
        case 20216:
            ch->AddAffect(AFFECT_MOUNT, POINT_MOV_SPEED, 30, length, 0, true, true);
            break;

        case 20205:
        case 20206:
        case 20207:
        case 20208:
        case 20214:
        case 20217:
            ch->AddAffect(AFFECT_MOUNT, POINT_MOV_SPEED, 40, length, 0, true, true);
            break;

        case 20209:
        case 20210:
        case 20211:
        case 20212:
        case 20215:
        case 20218:
            ch->AddAffect(AFFECT_MOUNT, POINT_MOV_SPEED, 50, length, 0, true, true);
            break;
        }
    }

    return 0;
}

int pc_mount_bonus(lua_State *L)
{
    uint8_t applyOn = static_cast<uint8_t>(lua_tonumber(L, 1));
    long value = static_cast<long>(lua_tonumber(L, 2));
    long duration = static_cast<long>(lua_tonumber(L, 3));

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != ch)
    {
        ch->RemoveAffect(AFFECT_MOUNT_BONUS);
        ch->AddAffect(AFFECT_MOUNT_BONUS, GetApplyPoint(applyOn), value, duration, 0, false);
    }

    return 0;
}

int pc_unmount(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    Unmount(ch);
    return 0;
}

int pc_get_horse_level(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetHorseLevel());
    return 1;
}

int pc_get_horse_hp(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch->GetHorseLevel())
        lua_pushnumber(L, ch->GetHorseHealth());
    else
        lua_pushnumber(L, 0);

    return 1;
}

int pc_get_horse_stamina(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch->GetHorseLevel())
        lua_pushnumber(L, ch->GetHorseStamina());
    else
        lua_pushnumber(L, 0);

    return 1;
}

int pc_is_horse_alive(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, ch->GetHorseLevel() > 0 && ch->GetHorseHealth() > 0);
    return 1;
}

int pc_revive_horse(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->ReviveHorse();
    return 0;
}

int pc_have_map_scroll(lua_State *L)
{
    if (!lua_isstring(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    const char *szMapName = lua_tostring(L, 1);
    const TMapRegion *region = SECTREE_MANAGER::instance().FindRegionByPartialName(szMapName);

    if (!region)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    bool bFind = false;
    for (int iCell = 0; iCell < INVENTORY_MAX_NUM; iCell++)
    {
        CItem *item = ch->GetInventoryItem(iCell);
        if (!item)
            continue;

        if (item->GetItemType() == ITEM_USE && item->GetSubType() == USE_TALISMAN &&
            (item->GetValue(0) == 1 || item->GetValue(0) == 2))
        {
            if (item->GetSocket(2) == region->index)
            {
                bFind = true;
                break;
            }
        }
    }

    lua_pushboolean(L, bFind);
    return 1;
}

int pc_get_war_map(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetWarMap() ? ch->GetWarMap()->GetMapIndex() : 0);
    return 1;
}

int pc_have_pos_scroll(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isnumber(L, 3))
    {
        SPDLOG_ERROR("invalid (map, x, y) position");
        lua_pushboolean(L, 0);
        return 1;
    }

    if (!lua_isnumber(L, 4))
    {
        SPDLOG_ERROR("invalid radius");
        lua_pushboolean(L, 0);
        return 1;
    }

    uint32_t index = (uint32_t)lua_tonumber(L, 1);
    int x = (int)lua_tonumber(L, 2);
    int y = (int)lua_tonumber(L, 3);
    float r = (float)lua_tonumber(L, 4);

    bool bFind = false;
    for (int iCell = 0; iCell < INVENTORY_MAX_NUM; iCell++)
    {
        CItem *item = ch->GetInventoryItem(iCell);
        if (!item)
            continue;

        if (item->GetItemType() == ITEM_USE && item->GetSubType() == USE_TALISMAN &&
            (item->GetValue(0) == 1 || item->GetValue(0) == 2) && item->GetValue(2) == index)
        {
            int item_x = item->GetSocket(0);
            int item_y = item->GetSocket(1);
            if ((x - item_x) * (x - item_x) + (y - item_y) * (y - item_y) < r * r)
            {
                bFind = true;
                break;
            }
        }
    }

    lua_pushboolean(L, bFind);
    return 1;
}

int pc_get_equip_refine_level(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int cell = (int)lua_tonumber(L, 1);
    if (cell < 0 || cell >= WEAR_MAX_NUM)
    {
        SPDLOG_ERROR("invalid wear position %d", cell);
        lua_pushnumber(L, 0);
        return 1;
    }

    CItem *item = ch->GetWear(cell);
    if (!item)
    {
        lua_pushnumber(L, 0);
        return 1;
    }

    lua_pushnumber(L, item->GetRefineLevel());
    return 1;
}

int pc_refine_equip(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2))
    {
        SPDLOG_ERROR("invalid argument");
        lua_pushboolean(L, 0);
        return 1;
    }

    int cell = (int)lua_tonumber(L, 1);
    int level_limit = (int)lua_tonumber(L, 2);
    int pct = lua_isnumber(L, 3) ? (int)lua_tonumber(L, 3) : 100;

    CItem *item = ch->GetWear(cell);
    if (!item)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    if (item->GetRefinedVnum() == 0)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    if (item->GetRefineLevel() > level_limit)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    if (pct == 100 || Random::get(1, 100) <= pct)
    {
        // 개량 성공
        lua_pushboolean(L, 1);

        CItem *pkNewItem = ITEM_MANAGER::instance().CreateItem(item->GetRefinedVnum());
        if (pkNewItem)
        {
            for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
                if (!item->GetSocket(i))
                    break;
                else
                    pkNewItem->SetSocket(i, 1);

            int set = 0;
            for (int i = 0; i < ITEM_SOCKET_MAX_NUM; i++)
            {
                int32_t socket = item->GetSocket(i);
                if (socket > 2 && socket != 28960)
                {
                    pkNewItem->SetSocket(set++, socket);
                }
            }

            item->CopyAttributeTo(pkNewItem);

            ITEM_MANAGER::instance().RemoveItem(item, "REMOVE (REFINE SUCCESS)");
            pkNewItem->AddToCharacter(ch, ItemPosition(EQUIPMENT, cell));

            ITEM_MANAGER::instance().FlushDelayedSave(pkNewItem);
            ;
            LogManager::instance().ItemLog(ch, pkNewItem, "REFINE SUCCESS (QUEST)", pkNewItem->GetName());
        }
    }
    else
    {
        // 개량 실패
        lua_pushboolean(L, 0);
    }

    return 1;
}

int pc_get_skill_level(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("invalid argument");
        lua_pushnumber(L, 0);
        return 1;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    uint32_t dwVnum = (uint32_t)lua_tonumber(L, 1);
    lua_pushnumber(L, ch->GetSkillLevel(dwVnum));

    return 1;
}

int pc_aggregate_monster(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->AggregateMonster(5000);
    return 0;
}

int pc_forget_my_attacker(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->ForgetMyAttacker();
    return 0;
}

int pc_attract_ranger(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->AttractRanger();
    return 0;
}

int pc_select_pid(lua_State *L)
{
    uint32_t pid = (uint32_t)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *new_ch = g_pCharManager->FindByPID(pid);

    if (new_ch)
    {
        CQuestManager::instance().GetPC(new_ch->GetPlayerID());

        lua_pushnumber(L, ch->GetPlayerID());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_select_vid(lua_State *L)
{
    uint32_t vid = (uint32_t)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *new_ch = g_pCharManager->Find(vid);

    if (new_ch)
    {
        CQuestManager::instance().GetPC(new_ch->GetPlayerID());

        lua_pushnumber(L, (uint32_t)ch->GetVID());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_sex(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, GetSexByRace(ch->GetRaceNum())); /* 0==MALE, 1==FEMALE */
    return 1;
}

int pc_is_engaged(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, marriage::CManager::instance().IsEngaged(ch->GetPlayerID()));
    return 1;
}

int pc_is_married(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, marriage::CManager::instance().IsMarried(ch->GetPlayerID()));
    return 1;
}

int pc_is_engaged_or_married(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, marriage::CManager::instance().IsEngagedOrMarried(ch->GetPlayerID()));
    return 1;
}

int pc_is_gm(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushboolean(L, ch->GetGMLevel() >= GM_HIGH_WIZARD);

    return 1;
}

int pc_get_gm_level(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetGMLevel());
    return 1;
}

int pc_mining(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *npc = CQuestManager::instance().GetCurrentNPCCharacterPtr();
    ch->mining(npc);
    return 0;
}

int pc_diamond_refine(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    Gold cost = (Gold)lua_tonumber(L, 1);
    int pct = (int)lua_tonumber(L, 2);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *npc = CQuestManager::instance().GetCurrentNPCCharacterPtr();
    CItem *item = CQuestManager::instance().GetCurrentItem();

    if (item)
        lua_pushboolean(L, mining::OreRefine(ch, npc, item, cost, pct, nullptr));
    else
        lua_pushboolean(L, 0);

    return 1;
}

int pc_ore_refine(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    Gold cost = (Gold)lua_tonumber(L, 1);
    int pct = (int)lua_tonumber(L, 2);
    int metinstone_cell = (int)lua_tonumber(L, 3);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *npc = CQuestManager::instance().GetCurrentNPCCharacterPtr();
    CItem *item = CQuestManager::instance().GetCurrentItem();

    CItem *metinstone_item = ch->GetInventoryItem(metinstone_cell);

    if (item && metinstone_item)
        lua_pushboolean(L, mining::OreRefine(ch, npc, item, cost, pct, metinstone_item));
    else
        lua_pushboolean(L, 0);

    return 1;
}

int pc_clear_skill(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
        return 0;

    ch->ClearSkill();

    return 0;
}

int pc_clear_sub_skill(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
        return 0;

    ch->ClearSubSkill();

    return 0;
}

int pc_set_skill_point(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        return 0;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int newPoint = (int)lua_tonumber(L, 1);

    ch->SetPoint(POINT_SKILL, newPoint);
    ch->PointChange(POINT_SKILL, 0);
    ch->ComputePoints();
    ch->ComputeMountPoints();
    ch->PointsPacket();

    return 0;
}

// RESET_ONE_SKILL
int pc_clear_one_skill(lua_State *L)
{
    int vnum = (int)lua_tonumber(L, 1);
    SPDLOG_TRACE("%d skill clear", vnum);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch == nullptr)
    {
        SPDLOG_TRACE("skill clear fail");
        lua_pushnumber(L, 0);
        return 1;
    }

    SPDLOG_TRACE("%d skill clear", vnum);

    ch->ResetOneSkill(vnum);

    return 0;
}

// END_RESET_ONE_SKILL

int pc_is_clear_skill_group(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    lua_pushboolean(L, ch->GetQuestFlag("skill_group_clear.clear") == 1);

    return 1;
}

int pc_save_exit_location(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    ch->SaveExitLocation();

    return 0;
}

//텔레포트
int pc_teleport(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    const char *arg1 = lua_tostring(L, 1);

    CHARACTER *tch = g_pCharManager->FindPC(arg1);

    if (!tch)
    {
        if (nullptr == g_pCharManager->FindPC(arg1))
        {
            ch->ChatPacket(CHAT_TYPE_INFO, "There is no one by that name");
        }

        lua_pushnumber(L, 0);

        return 1;
    }
    ch->ChatPacket(CHAT_TYPE_INFO, "You warp to %d ( %d, %d )", tch->GetMapIndex(), tch->GetX(), tch->GetY());
    ch->WarpSet(tch->GetMapIndex(), tch->GetX(), tch->GetY());
    ch->Stop();
    lua_pushnumber(L, 1);
    return 1;
}

int pc_set_skill_level(lua_State *L)
{
    uint32_t dwVnum = (uint32_t)lua_tonumber(L, 1);
    uint8_t byLev = (uint8_t)lua_tonumber(L, 2);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    ch->SetSkillLevel(dwVnum, byLev);

    ch->SkillLevelPacket();

    return 0;
}

int pc_give_polymorph_book(lua_State *L)
{
    if (lua_isnumber(L, 1) != 1 && lua_isnumber(L, 2) != 1 && lua_isnumber(L, 3) != 1 && lua_isnumber(L, 4) != 1)
    {
        SPDLOG_ERROR("Wrong Quest Function Arguments: pc_give_polymorph_book");
        return 0;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    CPolymorphUtils::instance().GiveBook(ch, (uint32_t)lua_tonumber(L, 1), (uint32_t)lua_tonumber(L, 2),
                                         (uint8_t)lua_tonumber(L, 3), (uint8_t)lua_tonumber(L, 4));

    return 0;
}

int pc_upgrade_polymorph_book(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CItem *pItem = CQuestManager::instance().GetCurrentItem();

    bool ret = CPolymorphUtils::instance().BookUpgrade(ch, pItem);

    lua_pushboolean(L, ret);

    return 1;
}

int pc_get_premium_remain_sec(lua_State *L)
{
    int remain_seconds = 0;
    int premium_type = 0;
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("wrong premium index (is not number)");
        return 0;
    }

    premium_type = (int)lua_tonumber(L, 1);
    switch (premium_type)
    {
    case PREMIUM_EXP:
    case PREMIUM_ITEM:
    case PREMIUM_SAFEBOX:
    case PREMIUM_AUTOLOOT:
    case PREMIUM_FISH_MIND:
    case PREMIUM_MARRIAGE_FAST:
    case PREMIUM_GOLD:
    case PREMIUM_SHOP_DOUBLE_UP:
    case PREMIUM_VOTE:
        break;

    default:
        SPDLOG_ERROR("wrong premium index %d", premium_type);
        return 0;
    }

    remain_seconds = ch->GetPremiumRemainSeconds(premium_type);

    lua_pushnumber(L, remain_seconds);
    return 1;
}

int pc_is_block_mode(lua_State *L)
{
    if (lua_isnumber(L, 1) != 1)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    uint32_t blockMode = lua_tonumber(L, 1);

    lua_pushboolean(L, ch->IsBlockMode(blockMode));
    return 1;
}

int pc_is_change_name(lua_State *L)
{
    auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    char szQuery[1024];
    snprintf(szQuery, sizeof(szQuery), "SELECT change_name FROM player WHERE id=%d", ch->GetPlayerID());

    std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
    if (msg->Get()->uiNumRows > 0)
    {
        MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);

        int is = 0;
        str_to_number(is, row[0]);
        lua_pushboolean(L, is);
        return 1;
    }

    lua_pushboolean(L, 0);
    return 1;
}

int pc_set_change_name(lua_State *L)
{
    auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    char szQuery[1024];
    snprintf(szQuery, sizeof(szQuery), "UPDATE player SET change_name = %d WHERE id=%d",
             lua_isnumber(L, 1) ? (int)lua_tonumber(L, 1) : 1, ch->GetPlayerID());

    std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
    if (msg->Get()->uiAffectedRows > 0)
    {
        ch->GetDesc()->DelayedDisconnect(1);
        lua_pushboolean(L, 1);
        return 1;
    }

    lua_pushboolean(L, 0);
    return 1;
}

int pc_is_change_empire(lua_State *L)
{
    auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    char szQuery[1024];
    snprintf(szQuery, sizeof(szQuery), "SELECT change_empire FROM player WHERE id=%d", ch->GetPlayerID());

    std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
    if (msg->Get()->uiNumRows > 0)
    {
        MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);

        int is = 0;
        str_to_number(is, row[0]);
        lua_pushboolean(L, is);
        return 1;
    }

    lua_pushboolean(L, 0);
    return 1;
}

int pc_set_change_empire(lua_State *L)
{
    auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    char szQuery[1024];
    snprintf(szQuery, sizeof(szQuery), "UPDATE player SET change_empire = %d WHERE id=%d",
             lua_isnumber(L, 1) ? (int)lua_tonumber(L, 1) : 1, ch->GetPlayerID());

    std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
    if (msg->Get()->uiAffectedRows > 0)
    {
        ch->GetDesc()->DelayedDisconnect(2);
        lua_pushboolean(L, 1);
        return 1;
    }

    lua_pushboolean(L, 0);
    return 1;
}

int pc_delayed_disconnect(lua_State *L)
{
    auto ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    if (!lua_isnumber(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    ch->GetDesc()->DelayedDisconnect(lua_tonumber(L, 1), "QUEST_DELAYED_DISCONNECT");
    lua_pushboolean(L, 0);
    return 1;
}

int pc_is_dead(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (ch != nullptr)
    {
        lua_pushboolean(L, ch->IsDead());
        return 1;
    }

    lua_pushboolean(L, true);

    return 1;
}

int pc_reset_status(lua_State *L)
{
    if (lua_isnumber(L, 1) == 1)
    {
        int idx = (int)lua_tonumber(L, 1);

        if (idx >= 0 && idx < 4)
        {
            CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
            int point = POINT_NONE;
            char buf[128];

            switch (idx)
            {
            case 0:
                point = POINT_HT;
                break;
            case 1:
                point = POINT_IQ;
                break;
            case 2:
                point = POINT_ST;
                break;
            case 3:
                point = POINT_DX;
                break;
            default:
                lua_pushboolean(L, false);
                return 1;
            }

            int old_val = ch->GetRealPoint(point);
            int old_stat = ch->GetRealPoint(POINT_STAT);

            ch->SetRealPoint(point, 1);
            ch->SetPoint(point, ch->GetRealPoint(point));

            ch->PointChange(POINT_STAT, old_val - 1);

            if (point == POINT_HT)
            {
                uint8_t job = ch->GetJob();
                ch->SetRandomHP((ch->GetLevel() - 1) * Random::get(JobInitialPoints[job].hp_per_lv_begin,
                                                                   JobInitialPoints[job].hp_per_lv_end));
            }
            else if (point == POINT_IQ)
            {
                uint8_t job = ch->GetJob();
                ch->SetRandomSP((ch->GetLevel() - 1) * Random::get(JobInitialPoints[job].sp_per_lv_begin,
                                                                   JobInitialPoints[job].sp_per_lv_end));
            }

            ch->ComputePoints();
            ch->ComputeMountPoints();
            ch->PointsPacket();

            if (point == POINT_HT)
            {
                ch->PointChange(POINT_HP, ch->GetMaxHP() - ch->GetHP());
            }
            else if (point == POINT_IQ)
            {
                ch->PointChange(POINT_SP, ch->GetMaxSP() - ch->GetSP());
            }

            switch (idx)
            {
            case 0:
                std::snprintf(buf, sizeof(buf), "reset ht(%d)->1 stat_point(%d)->(%lld)", old_val, old_stat,
                              ch->GetRealPoint(POINT_STAT));
                break;
            case 1:
                std::snprintf(buf, sizeof(buf), "reset iq(%d)->1 stat_point(%d)->(%lld)", old_val, old_stat,
                              ch->GetRealPoint(POINT_STAT));
                break;
            case 2:
                std::snprintf(buf, sizeof(buf), "reset st(%d)->1 stat_point(%d)->(%lld)", old_val, old_stat,
                              ch->GetRealPoint(POINT_STAT));
                break;
            case 3:
                std::snprintf(buf, sizeof(buf), "reset dx(%d)->1 stat_point(%d)->(%lld)", old_val, old_stat,
                              ch->GetRealPoint(POINT_STAT));
                break;
            }

            LogManager::instance().CharLog(ch, 0, "RESET_ONE_STATUS", buf);

            lua_pushboolean(L, true);
            return 1;
        }
    }

    lua_pushboolean(L, false);
    return 1;
}

int pc_get_ht(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetRealPoint(POINT_HT));
    return 1;
}

int pc_give_skilltree_points(lua_State *L)
{
    if (lua_isnumber(L, 1) == false)
        return 1;

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    ch->PointChange(POINT_SKILLTREE_POINTS, static_cast<PointValue>(lua_tonumber(L, 1)));
    return 1;
}

int pc_set_ht(lua_State *L)
{
    if (lua_isnumber(L, 1) == false)
        return 1;

    int newPoint = (int)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int usedPoint = newPoint - ch->GetRealPoint(POINT_HT);
    ch->SetRealPoint(POINT_HT, newPoint);
    ch->PointChange(POINT_HT, 0);
    ch->PointChange(POINT_STAT, -usedPoint);
    ch->ComputePoints();
    ch->ComputeMountPoints();
    ch->PointsPacket();
    return 1;
}

int pc_get_iq(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetRealPoint(POINT_IQ));
    return 1;
}

int pc_set_iq(lua_State *L)
{
    if (lua_isnumber(L, 1) == false)
        return 1;

    int newPoint = (int)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int usedPoint = newPoint - ch->GetRealPoint(POINT_IQ);
    ch->SetRealPoint(POINT_IQ, newPoint);
    ch->PointChange(POINT_IQ, 0);
    ch->PointChange(POINT_STAT, -usedPoint);
    ch->ComputePoints();
    ch->ComputeMountPoints();
    ch->PointsPacket();
    return 1;
}

int pc_get_st(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetRealPoint(POINT_ST));
    return 1;
}

int pc_set_st(lua_State *L)
{
    if (lua_isnumber(L, 1) == false)
        return 1;

    int newPoint = (int)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int usedPoint = newPoint - ch->GetRealPoint(POINT_ST);
    ch->SetRealPoint(POINT_ST, newPoint);
    ch->PointChange(POINT_ST, 0);
    ch->PointChange(POINT_STAT, -usedPoint);
    ch->ComputePoints();
    ch->ComputeMountPoints();
    ch->PointsPacket();
    return 1;
}

int pc_get_dx(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    lua_pushnumber(L, ch->GetRealPoint(POINT_DX));
    return 1;
}

int pc_set_dx(lua_State *L)
{
    if (lua_isnumber(L, 1) == false)
        return 1;

    int newPoint = (int)lua_tonumber(L, 1);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int usedPoint = newPoint - ch->GetRealPoint(POINT_DX);
    ch->SetRealPoint(POINT_DX, newPoint);
    ch->PointChange(POINT_DX, 0);
    ch->PointChange(POINT_STAT, -usedPoint);
    ch->ComputePoints();
    ch->ComputeMountPoints();
    ch->PointsPacket();
    return 1;
}

int pc_is_near_vid(lua_State *L)
{
    if (lua_isnumber(L, 1) != 1 || lua_isnumber(L, 2) != 1)
    {
        lua_pushboolean(L, false);
    }
    else
    {
        CHARACTER *pMe = CQuestManager::instance().GetCurrentCharacterPtr();
        CHARACTER *pOther = g_pCharManager->Find((uint32_t)lua_tonumber(L, 1));

        if (pMe != nullptr && pOther != nullptr)
        {
            lua_pushboolean(L, (DISTANCE_APPROX(pMe->GetX() - pOther->GetX(), pMe->GetY() - pOther->GetY()) <
                                (int)lua_tonumber(L, 2) * 100));
        }
        else
        {
            lua_pushboolean(L, false);
        }
    }

    return 1;
}

int pc_get_socket_items(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    lua_newtable(L);

    if (pChar == nullptr)
        return 1;

    int idx = 1;

    // 용혼석 슬롯은 할 필요 없을 듯.
    // 이 함수는 탈석서용 함수인 듯 하다.
    for (int i = 0; i < INVENTORY_MAX_NUM; i++)
    {
        CItem *pItem = pChar->GetInventoryItem(i);

        if (pItem != nullptr)
        {
            if (pItem->IsEquipped() == false)
            {
                int j = 0;
                for (; j < ITEM_SOCKET_MAX_NUM; j++)
                {
                    long socket = pItem->GetSocket(j);

                    if (socket > 2 && socket != ITEM_BROKEN_METIN_VNUM)
                    {
                        const TItemTable *pItemInfo = ITEM_MANAGER::instance().GetTable(socket);
                        if (pItemInfo != nullptr)
                        {
                            if (pItemInfo->bType == ITEM_METIN)
                                break;
                        }
                    }
                }

                if (j >= ITEM_SOCKET_MAX_NUM)
                    continue;

                lua_newtable(L);

                {
                    lua_pushstring(L, pItem->GetName());
                    lua_rawseti(L, -2, 1);

                    lua_pushnumber(L, i);
                    lua_rawseti(L, -2, 2);
                }

                lua_rawseti(L, -2, idx++);
            }
        }
    }

    return 1;
}


int pc_get_empty_inventory_count(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->GetEmptyInventoryCount(3));
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_empty_normal_inventory_count(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->GetEmptyNormalInventoryCount(3));
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_count_empty_inventory(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->CountEmptyInventory());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_logoff_interval(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->GetLogOffInterval());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_last_play(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->GetLastPlay());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_player_id(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        lua_pushnumber(L, pChar->GetPlayerID());
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_account_id(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (pChar != nullptr)
    {
        if (pChar->GetDesc() != nullptr)
        {
            lua_pushnumber(L, pChar->GetDesc()->GetAccountTable().id);
            return 1;
        }
    }

    lua_pushnumber(L, 0);
    return 1;
}

int pc_get_account(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        if (nullptr != pChar->GetDesc())
        {
            lua_pushstring(L, pChar->GetDesc()->GetLogin().c_str());
            return 1;
        }
    }

    lua_pushstring(L, "");
    return 1;
}

int pc_is_riding(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        bool is_riding = pChar->IsRiding();

        lua_pushboolean(L, is_riding);

        return 1;
    }

    lua_pushboolean(L, false);
    return 1;
}

int pc_get_special_ride_vnum(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        CItem *CostumeMount = pChar->GetWear(WEAR_COSTUME_MOUNT);

        if (nullptr != CostumeMount)
        {
            if (ITEM_COSTUME == CostumeMount->GetItemType() && COSTUME_MOUNT == CostumeMount->GetSubType())
            {
                lua_pushnumber(L, CostumeMount->GetVnum());
                lua_pushnumber(L, CostumeMount->GetSocket(2));
                return 2;
            }
        }
    }

    lua_pushnumber(L, 0);
    lua_pushnumber(L, 0);

    return 2;
}

int pc_can_warp(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        lua_pushboolean(L, pChar->CanWarp());
    }
    else
    {
        lua_pushboolean(L, false);
    }

    return 1;
}

int pc_dec_skill_point(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        pChar->PointChange(POINT_SKILL, -1);
    }

    return 0;
}

int pc_get_skill_point(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();

    if (nullptr != pChar)
    {
        lua_pushnumber(L, pChar->GetPoint(POINT_SKILL));
    }
    else
    {
        lua_pushnumber(L, 0);
    }

    return 1;
}

int pc_get_channel_id(lua_State *L)
{
    lua_pushnumber(L, gConfig.channel);

    return 1;
}

int pc_give_poly_marble(lua_State *L)
{
    const int dwVnum = (int)lua_tonumber(L, 1);

    const auto *MobInfo = CMobManager::instance().Get(dwVnum);

    if (nullptr == MobInfo)
    {
        lua_pushboolean(L, false);
        return 1;
    }

    if (0 == MobInfo->dwPolymorphItemVnum)
    {
        lua_pushboolean(L, false);
        return 1;
    }

    CItem *item = ITEM_MANAGER::instance().CreateItem(MobInfo->dwPolymorphItemVnum);

    if (nullptr == item)
    {
        lua_pushboolean(L, false);
        return 1;
    }

    item->SetSocket(0, dwVnum);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    int iEmptyCell = ch->GetEmptyInventory(item);

    if (-1 == iEmptyCell)
    {
        M2_DESTROY_ITEM(item);
        lua_pushboolean(L, false);
        return 1;
    }

    item->AddToCharacter(ch, TItemPos(INVENTORY, iEmptyCell));

    const PC *pPC = CQuestManager::instance().GetCurrentPC();

    LogManager::instance().QuestRewardLog(pPC->GetCurrentQuestName().c_str(), ch->GetPlayerID(), ch->GetLevel(),
                                          MobInfo->dwPolymorphItemVnum, dwVnum);

    lua_pushboolean(L, true);

    return 1;
}

int pc_get_sig_items(lua_State *L)
{
    uint32_t group_vnum = (uint32_t)lua_tonumber(L, 1);
    const CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    int count = 0;
    for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
    {
        if (ch->GetInventoryItem(i) != nullptr && ch->GetInventoryItem(i)->GetSIGVnum() == group_vnum)
        {
            lua_pushnumber(L, ch->GetInventoryItem(i)->GetID());
            count++;
        }
    }

    return count;
}

int pc_charge_cash(lua_State *L)
{
    int amount = lua_isnumber(L, 1) ? (int)lua_tonumber(L, 1) : 0;
    std::string strChargeType = lua_isstring(L, 2) ? lua_tostring(L, 2) : "";

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!ch || !ch->GetDesc())
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    ch->ChangeCash(amount, strChargeType, true);
    lua_pushboolean(L, 1);
    return 1;
}

int pc_give_award(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isstring(L, 3))
    {
        SPDLOG_ERROR("QUEST give award call error : wrong argument");
        lua_pushnumber(L, 0);
        return 1;
    }

    uint32_t dwVnum = (int)lua_tonumber(L, 1);

    int icount = (int)lua_tonumber(L, 2);

    SPDLOG_TRACE("QUEST [award] item %d to login %s", dwVnum, ch->GetDesc()->GetLogin().c_str());

    DBManager::instance().Query(
        "INSERT INTO item_award (login, vnum, count, given_time, why, mall)select '%s', %d, %d, now(), '%s', 1 from "
        "DUAL where not exists (select login, why from item_award where login = '%s' and why  = '%s') ;",
        ch->GetDesc()->GetLogin().c_str(), dwVnum, icount, lua_tostring(L, 3), ch->GetDesc()->GetLogin().c_str(),
        lua_tostring(L, 3));

    lua_pushnumber(L, 0);
    return 1;
}

int pc_give_award_socket(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isstring(L, 3) || !lua_isstring(L, 4) ||
        !lua_isstring(L, 5) || !lua_isstring(L, 6))
    {
        SPDLOG_ERROR("QUEST give award call error : wrong argument");
        lua_pushnumber(L, 0);
        return 1;
    }

    uint32_t dwVnum = (int)lua_tonumber(L, 1);

    int icount = (int)lua_tonumber(L, 2);

    SPDLOG_TRACE("QUEST [award] item %d to login %s", dwVnum, ch->GetDesc()->GetLogin().c_str());

    DBManager::instance().Query("INSERT INTO item_award (login, vnum, count, given_time, why, mall, socket0, socket1, "
                                "socket2)select '%s', %d, %d, now(), '%s', 1, %s, %s, %s from DUAL where not exists "
                                "(select login, why from item_award where login = '%s' and why  = '%s') ;",
                                ch->GetDesc()->GetLogin().c_str(), dwVnum, icount, lua_tostring(L, 3),
                                lua_tostring(L, 4), lua_tostring(L, 5), lua_tostring(L, 6),
                                ch->GetDesc()->GetLogin().c_str(), lua_tostring(L, 3));

    lua_pushnumber(L, 0);
    return 1;
}

int pc_get_killee_drop_pct(lua_State *L)
{
    CHARACTER *pChar = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *pKillee = pChar->GetQuestNPC();

    int iDeltaPercent, iRandRange;
    if (nullptr == pKillee || !ITEM_MANAGER::instance().GetDropPct(pKillee, pChar, iDeltaPercent, iRandRange))
    {
        SPDLOG_ERROR("killee is null");
        lua_pushnumber(L, -1);
        lua_pushnumber(L, -1);

        return 2;
    }

    lua_pushnumber(L, iDeltaPercent);
    lua_pushnumber(L, iRandRange);

    return 2;
}

int pc_specific_effect(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (lua_isstring(L, 1))
    {
        ch->SpecificEffectPacket(lua_tostring(L, 1));
    }
    return 0;
}

int pc_start_duel(lua_State *L)
{
    uint32_t vid = (int)lua_tonumber(L, 1);
    if (!vid)
    {
        lua_pushnumber(L, 0);
        return 0;
    }

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *pkVictim = g_pCharManager->Find(vid);

    if (!pkVictim)
    {
        lua_pushnumber(L, 0);
        return 0;
    }

    if (pkVictim->IsNPC())
    {
        lua_pushnumber(L, 0);
        return 0;
    }

    CPVPManager::instance().Insert(ch, pkVictim);
    CPVPManager::instance().Insert(pkVictim, ch);

    lua_pushnumber(L, 0);
    return 1;
}

/*
    @author MartPwnS
    @desc Stuns the current CurrentCharacter.
    @usage pc.stun((time as int in seconds [2 seconds]), (reason as string)[QUEST_STUN]), (forced as boolean)[false])
*/
int pc_stun_player(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!ch)
    {
        SPDLOG_ERROR("no valid Character found [GetCurrentCharacterPtr() == NULL");
        return 0;
    }

    uint32_t time = (lua_isnumber(L, 1) && (int)lua_tonumber(L, 1) > 0) ? (int)lua_tonumber(L, 1) : 2;
    // 2 seconds by default
    const char *reason = (lua_isstring(L, 2)) ? lua_tostring(L, 2) : "QUEST_STUN";
    int IMMUNE_FLAG = (lua_isboolean(L, 3) && lua_toboolean(L, 3)) ? 0 : IMMUNE_STUN;

    SkillAttackAffect(ch, 1000, IMMUNE_FLAG, AFFECT_STUN, POINT_NONE, 0, time, reason);

    return 0;
}

int pc_acce_open_combine(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        SPDLOG_ERROR("No character on a pc function!");
        return 0;
    }

    if (!ch->IsAcceWindowOpen())
    {
        ch->SetAcceWindowType(ACCE_SLOT_TYPE_COMBINE);
        ch->ChatPacket(CHAT_TYPE_COMMAND, "ShowAcceCombineDialog");
    }
    else
    {
        SendI18nChatPacket(ch, CHAT_TYPE_INFO, "ACCE_WINDOW_ALREADY_OPEN");
    }
    return 0;
}

int pc_acce_open_absorb(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        SPDLOG_ERROR("No character on a pc function!");
        return 0;
    }

    if (!ch->IsAcceWindowOpen())
    {
        ch->SetAcceWindowType(ACCE_SLOT_TYPE_ABSORB);
        ch->ChatPacket(CHAT_TYPE_COMMAND, "ShowAcceAbsorbDialog");
    }
    else
    {
        SendI18nChatPacket(ch, CHAT_TYPE_INFO, "ACCE_WINDOW_ALREADY_OPEN");
    }

    return 0;
}

#ifdef ENABLE_MOVE_COSTUME_ATTR
int pc_open_item_combination(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        SPDLOG_ERROR("No character on a pc function!");
        return 0;
    }

    ch->ChatPacket(CHAT_TYPE_COMMAND, "ShowItemCombinationDialog");

    return 0;
}
#endif

#ifdef ENABLE_GEM_SYSTEM
int pc_gem_refine(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    int cost = (int)lua_tonumber(L, 1);
    int metinstone_cell = (int)lua_tonumber(L, 2);

    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    CHARACTER *npc = CQuestManager::instance().GetCurrentNPCCharacterPtr();
    CItem *item = CQuestManager::instance().GetCurrentItem();

    CItem *metinstone_item = ch->GetInventoryItem(metinstone_cell);

    if (item && metinstone_item)
        lua_pushboolean(L, mining::GemRefine(ch, npc, item, cost, metinstone_item));
    else
        lua_pushboolean(L, 0);

    return 1;
}
#endif

int pc_reencode_view(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        SPDLOG_ERROR("No character on a pc function!");
        return 0;
    }

    ch->ViewReencode();
    return 0;
}

int pc_go_home(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
    {
        SPDLOG_ERROR("No character on a pc function!");
        return 0;
    }

    ch->GoHome();
    return 0;
}

#ifdef __FAKE_PC__
int pc_set_fakepc_name(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (!check_name(lua_tostring(L, 1)))
    {
        lua_pushboolean(L, 0);
        return 1;
    }

    std::string stName = lua_tostring(L, 1);
    // stName += " (Ditto)";
    ch->FakePC_Owner_SetName(stName.c_str());

    if (CHARACTER *pkSupChr = ch->FakePC_Owner_GetSupporter())
    {
        CItem *pkSpawnItem = pkSupChr->FakePC_GetOwnerItem();
        long lX = pkSupChr->GetX(), lY = pkSupChr->GetY();

        M2_DESTROY_CHARACTER(pkSupChr);
        ch->FakePC_Owner_Spawn(lX, lY, pkSpawnItem);
    }

    lua_pushboolean(L, 1);
    return 1;
}
#endif
#ifdef __FAKE_PC__
int pc_spawn_copy(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (!ch)
        return 0;

    ch->SetQuestNPCID(0);

    int lMapIndex = ch->GetMapIndex();
    int lX = ch->GetX() + Random::get(-300, 300);
    int lY = ch->GetY() + Random::get(-300, 300);

    if (lua_isnumber(L, 1) && lua_isnumber(L, 2))
    {
        lX = lua_tonumber(L, 1) * 100;
        lY = lua_tonumber(L, 2) * 100;
    }

    auto npc = ch->FakePC_Owner_Spawn(lX, lY, nullptr, lua_isboolean(L, 3) ? lua_toboolean(L, 3) : false,
                                      lua_isboolean(L, 4) ? lua_toboolean(L, 4) : true);
    if (!npc)
        return 0;

    ch->SetQuestNPCID(npc->GetVID());

    return 0;
}
#endif

#ifdef ENABLE_BATTLE_PASS
int pc_update_dungeon_progress(lua_State *L)
{
    if (!lua_isnumber(L, 1))
    {
        SPDLOG_ERROR("arg1 must be number");
        return 0;
    }

    uint8_t bDungeonType = (int)lua_tonumber(L, 1);
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();

    if (ch)
    {
        uint8_t bBattlePassId = ch->GetBattlePassId();
        if (bBattlePassId)
        {
            uint32_t dwDungeonID, dwCount;
            if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, COMPLETE_DUNGEON, &dwDungeonID,
                                                                 &dwCount))
            {
                if ((dwDungeonID == bDungeonType || dwDungeonID >= DUNGEON_ID_MAX) && ch->GetMissionProgress(COMPLETE_DUNGEON, bBattlePassId) < dwCount)
                    ch->UpdateMissionProgress(COMPLETE_DUNGEON, bBattlePassId, 1, dwCount);
            }
        }
    }

    return 0;
}

int pc_open_battle_pass_ranking(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch)
    {
        uint32_t dwPlayerId = ch->GetPlayerID();
        uint8_t bIsGlobal = 1;

        db_clientdesc->DBPacketHeader(HEADER_GD_BATTLE_PASS_RANKING, ch->GetDesc()->GetHandle(),
                                      sizeof(uint32_t) + sizeof(uint8_t));
        db_clientdesc->Packet(&dwPlayerId, sizeof(uint32_t));
        db_clientdesc->Packet(&bIsGlobal, sizeof(uint8_t));
    }

    return 0;
}
#endif

int pc_has_active_dungeon(lua_State *L)
{
    CHARACTER *ch = CQuestManager::instance().GetCurrentCharacterPtr();
    if (ch)
    {
        auto& dngMgr = CDungeonManager::instance();
        const auto s = dngMgr.GetPlayerInfo(ch->GetPlayerID());
        if (s && dngMgr.FindByMapIndex<CDungeon>(s)) {
            lua_pushboolean(L, 1);
            return 1;
        } 
    }
    lua_pushboolean(L, 0);
    return 1;
}

void RegisterPCFunctionTable()
{
    luaL_reg pc_functions[] = {
        {"get_wear", pc_get_wear},
        {"get_player_id", pc_get_player_id},
        {"get_account_id", pc_get_account_id},
        {"get_account", pc_get_account},
        {"get_level", pc_get_level},
        {"get_hwid", pc_get_hwid},
        {"set_level", pc_set_level},
        {"get_next_exp", pc_get_next_exp},
        {"get_exp", pc_get_exp},
        {"get_job", pc_get_job},
        {"get_race", pc_get_race},
        {"change_sex", pc_change_sex},
        {"gethp", pc_get_hp},
        {"get_hp", pc_get_hp},
        {"getmaxhp", pc_get_max_hp},
        {"get_max_hp", pc_get_max_hp},
        {"getsp", pc_get_sp},
        {"get_sp", pc_get_sp},
        {"getmaxsp", pc_get_max_sp},
        {"get_max_sp", pc_get_max_sp},
        {"change_sp", pc_change_sp},
        {"getmoney", pc_get_money},
        {"get_money", pc_get_money},
        {"get_real_alignment", pc_get_real_alignment},
        {"get_alignment", pc_get_alignment},
        {"getweapon", pc_get_weapon},
        {"get_weapon", pc_get_weapon},
        {"getarmor", pc_get_armor},
        {"get_armor", pc_get_armor},
        {"getgold", pc_get_money},
        {"get_gold", pc_get_money},
        {"changegold", pc_change_money},
        {"changemoney", pc_change_money},
        {"changealignment", pc_change_alignment},
        {"change_gold", pc_change_money},
        {"change_money", pc_change_money},
        {"change_alignment", pc_change_alignment},
        {"getname", pc_get_name},
        {"get_name", pc_get_name},
        {"get_vid", pc_get_vid},
        {"get_pid", pc_get_pid},
        {"getplaytime", pc_get_playtime},
        {"get_playtime", pc_get_playtime},
        {"getleadership", pc_get_leadership},
        {"get_leadership", pc_get_leadership},
        {"getqf", pc_get_quest_flag},
        {"setqf", pc_set_quest_flag},
        {"delqf", pc_del_quest_flag},
        {"getf", pc_get_another_quest_flag},
        {"setf", pc_set_another_quest_flag},
        {"get_x", pc_get_x},
        {"get_y", pc_get_y},
        {"getx", pc_get_x},
        {"gety", pc_get_y},
        {"get_local_x", pc_get_local_x},
        {"get_local_y", pc_get_local_y},
        {"getcurrentmapindex", pc_get_current_map_index},
        {"get_map_index", pc_get_current_map_index},
        {"give_exp", pc_give_exp},
        {"give_exp_perc", pc_give_exp_perc},
        {"get_exp_perc", pc_get_exp_perc},
        {"give_exp2", pc_give_exp2},
        {"give_item", pc_give_item},
        {"give_item2", pc_give_or_drop_item},
        {"give_item2_select", pc_give_or_drop_item_and_select},
        {"give_gold", pc_give_gold},
        {"count_item", pc_count_item},
        {"remove_item", pc_remove_item},
        {"countitem", pc_count_item},
        {"removeitem", pc_remove_item},
        {"find_first_item", pc_find_first_item},
        {"reset_point", pc_reset_point},
        {"has_guild", pc_hasguild},
        {"hasguild", pc_hasguild},
        {"get_guild", pc_getguild},
        {"getguild", pc_getguild},
        {"isguildmaster", pc_isguildmaster},
        {"is_guild_master", pc_isguildmaster},
        {"destroy_guild", pc_destroy_guild},
        {"remove_from_guild", pc_remove_from_guild},
        {"in_dungeon", pc_in_dungeon},
        {"getempire", pc_get_empire},
        {"get_empire", pc_get_empire},
        {"get_skill_group", pc_get_skillgroup},
        {"set_skill_group", pc_set_skillgroup},
        {"change_skill_group", pc_change_skillgroup},

        {"warp", pc_warp},
        {"warp_force", pc_warp_force},
        {"warp_local", pc_warp_local},
        {"warp_exit", pc_warp_exit},
        {"set_warp_location", pc_set_warp_location},
        {"set_warp_location_local", pc_set_warp_location_local},
        {"get_start_location", pc_get_start_location},
        {"has_master_skill", pc_has_master_skill},
        {"set_part", pc_set_part},
        {"update_packet", pc_update_packet},
        {"get_part", pc_get_part},
        {"is_polymorphed", pc_is_polymorphed},
        {"remove_polymorph", pc_remove_polymorph},
        {"is_mount", pc_is_mount},
        {"polymorph", pc_polymorph},
        {"mount", pc_mount},
        {"mount_bonus", pc_mount_bonus},
        {"unmount", pc_unmount},
        {"warp_to_guild_war_observer_position", pc_warp_to_guild_war_observer_position},
        {"give_item_from_special_item_group", pc_give_item_from_special_item_group},
        {"learn_grand_master_skill", pc_learn_grand_master_skill},
        {"is_skill_book_no_delay", pc_is_skill_book_no_delay},
        {"remove_skill_book_no_delay", pc_remove_skill_book_no_delay},

        {"enough_inventory", pc_enough_inventory},
        {"get_horse_level", pc_get_horse_level}, // TO BE DELETED XXX
        {"is_horse_alive", pc_is_horse_alive},   // TO BE DELETED XXX
        {"revive_horse", pc_revive_horse},       // TO BE DELETED XXX
        {"have_pos_scroll", pc_have_pos_scroll},
        {"have_map_scroll", pc_have_map_scroll},
        {"get_war_map", pc_get_war_map},
        {"get_equip_refine_level", pc_get_equip_refine_level},
        {"refine_equip", pc_refine_equip},
        {"get_skill_level", pc_get_skill_level},
        {"aggregate_monster", pc_aggregate_monster},
        {"forget_my_attacker", pc_forget_my_attacker},
        {"pc_attract_ranger", pc_attract_ranger},
        {"select", pc_select_vid},
        {"get_sex", pc_get_sex},
        {"is_married", pc_is_married},
        {"is_engaged", pc_is_engaged},
        {"is_engaged_or_married", pc_is_engaged_or_married},
        {"is_gm", pc_is_gm},
        {"get_gm_level", pc_get_gm_level},
        {"mining", pc_mining},
        {"ore_refine", pc_ore_refine},
        {"diamond_refine", pc_diamond_refine},

        // RESET_ONE_SKILL
        {"clear_one_skill", pc_clear_one_skill},
        // END_RESET_ONE_SKILL

        {"clear_skill", pc_clear_skill},
        {"clear_sub_skill", pc_clear_sub_skill},
        {"set_skill_point", pc_set_skill_point},

        {"is_clear_skill_group", pc_is_clear_skill_group},

        {"save_exit_location", pc_save_exit_location},
        {"teleport", pc_teleport},

        {"set_skill_level", pc_set_skill_level},

        {"give_polymorph_book", pc_give_polymorph_book},
        {"upgrade_polymorph_book", pc_upgrade_polymorph_book},
        {"get_premium_remain_sec", pc_get_premium_remain_sec},

        {"is_block_mode", pc_is_block_mode},

        {"is_change_empire", pc_is_change_empire},
        {"set_change_empire", pc_set_change_empire},

        {"is_change_name", pc_is_change_name},
        {"set_change_name", pc_set_change_name},

        {"is_dead", pc_is_dead},

        {"reset_status", pc_reset_status},
        {"get_ht", pc_get_ht},
        {"set_ht", pc_set_ht},
        {"get_iq", pc_get_iq},
        {"set_iq", pc_set_iq},
        {"get_st", pc_get_st},
        {"set_st", pc_set_st},
        {"get_dx", pc_get_dx},
        {"set_dx", pc_set_dx},

        {"is_near_vid", pc_is_near_vid},

        {"get_socket_items", pc_get_socket_items},
        {"get_empty_inventory_count", pc_get_empty_inventory_count},

        {"get_logoff_interval", pc_get_logoff_interval},
        {"get_last_play", pc_get_last_play},

        {"is_riding", pc_is_riding},
        {"get_special_ride_vnum", pc_get_special_ride_vnum},

        {"can_warp", pc_can_warp},

        {"dec_skill_point", pc_dec_skill_point},
        {"get_skill_point", pc_get_skill_point},

        {"get_channel_id", pc_get_channel_id},

        {"give_poly_marble", pc_give_poly_marble},
        {"get_sig_items", pc_get_sig_items},

        {"charge_cash", pc_charge_cash},

        {"give_award", pc_give_award},               //일본 계정당 한번씩 금괴 지급
        {"give_award_socket", pc_give_award_socket}, //몰 인벤토리에 아이템 지급. 소켓 설정을 위한 함수.

        {"get_killee_drop_pct", pc_get_killee_drop_pct},
        /* mob_vnum.kill 이벤트에서 killee와 pc와의 level 차이, pc의 프리미엄 드랍률 등등을 고려한 아이템 드랍 확률.
         * return 값은 (분자, 분모).
         * (말이 복잡한데, CreateDropItem의 GetDropPct의 iDeltaPercent, iRandRange를 return한다고 보면 됨.)
         * (이 말이 더 어려울라나 ㅠㅠ)
         * 주의사항 : kill event에서만 사용할 것!
         */

        {"send_effect", pc_specific_effect}, // Think feb 14: Send a particular effect to a client (With client route)
        {"start_duel", pc_start_duel},       // Think - 01/04/14: Force a duel with a player
        {"stun", pc_stun_player},            // MartPwnS 02.11.14 Stun player for more details read the comments above.
        {"reencode_view", pc_reencode_view},

        {"acce_open_absorb", pc_acce_open_absorb},
        {"acce_open_combine", pc_acce_open_combine},
        {"go_home", pc_go_home},

#ifdef ENABLE_MOVE_COSTUME_ATTR
        {"open_item_combination", pc_open_item_combination},
#endif

#ifdef ENABLE_MESSENGER_BLOCK
        {"is_blocked", pc_is_blocked},
        {"is_friend", pc_is_friend},
#endif

#ifdef ENABLE_GEM_SYSTEM
        {"gem_refine", pc_gem_refine},
#endif
#ifdef __FAKE_PC__
        {"spawn_copy", pc_spawn_copy},
#endif

        {"give_skilltree_points", pc_give_skilltree_points},
        {"delayed_disconnect", pc_delayed_disconnect},

#ifdef ENABLE_BATTLE_PASS
        {"update_dungeon_progress", pc_update_dungeon_progress},
        {"open_battle_pass_ranking", pc_open_battle_pass_ranking},
#endif

        {"has_active_dungeon", pc_has_active_dungeon},
        {"get_empty_normal_inventory_count", pc_get_empty_normal_inventory_count},


        
        {nullptr, nullptr}};

    CQuestManager::instance().AddLuaFunctionTable("pc", pc_functions);
}
}; // namespace quest
