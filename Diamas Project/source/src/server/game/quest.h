#ifndef METIN2_SERVER_GAME_QUEST_H
#define METIN2_SERVER_GAME_QUEST_H
...
enum
{
    ...
    QUEST_BUY_EVENT,
    QUEST_RECEIVE_EVENT,
    QUEST_DUNGEON_STAGE_CHANGE_EVENT,
    QUEST_DUNGEON_ON_ELIMINATED_EVENT,
    QUEST_DEAD_EVENT,
    QUEST_ITEM_TO_ITEM_EVENT,
    QUEST_EVENT_COUNT
};
...
struct QuestState
{
    ...
    QuestState()
        : co(nullptr), ico(0), args(0), suspend_state(SUSPEND_STATE_NONE), iIndex(0), bStart(false), st(-1),
          _clock_value(0), _counter_value(0), item(0)
    {
    }
};
}

#endif /* METIN2_SERVER_GAME_QUEST_H */
