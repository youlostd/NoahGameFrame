...
CHorseRider::CHorseRider() { Initialize(); }

CHorseRider::~CHorseRider() { Destroy(); }
...
void CHorseRider::SetHorseData(const THorseInfo &crInfo) { m_Horse = crInfo; }
...
void CHorseRider::UpdateHorseStamina(int iStamina, bool bSend)
{
    ...
    if (GetHorseStamina() == 0 && IsHorseRiding()) { StopRiding(); }
...
{
    horserider_info *info = static_cast<horserider_info *>(event->info);
    ...
    int delta = THECORE_SECS_TO_PASSES(HORSE_STAMINA_CONSUME_INTERVAL);
...
EVENTFUNC(horse_stamina_regen_event)
{
    ...
    hr->UpdateHorseStamina(+1);
    int delta = THECORE_SECS_TO_PASSES(HORSE_STAMINA_REGEN_INTERVAL);
...
void CHorseRider::StartStaminaConsumeEvent()
{
    ...
    m_eventStaminaConsume = event_create(horse_stamina_consume_event, info,
                                         THECORE_SECS_TO_PASSES(HORSE_STAMINA_CONSUME_INTERVAL));
}
...
void CHorseRider::StartStaminaRegenEvent()
{
    ...
    info->hr = this;
    m_eventStaminaRegen = event_create(horse_stamina_regen_event, info,
                                       THECORE_SECS_TO_PASSES(HORSE_STAMINA_REGEN_INTERVAL));
}
...
{
    SPDLOG_INFO("HORSE DIE {:p} {:p}", (void*)m_eventStaminaRegen.get(), (void*)(m_eventStaminaConsume.get()));
...
uint8_t CHorseRider::GetHorseGrade() const
{
    uint8_t grade = 0;
